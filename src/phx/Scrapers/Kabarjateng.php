...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarjateng as KabarjatengProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarjateng extends NewsScraper
{

	const targetList = [
        "Jawa Tengah" => "http://www.kabarjateng.co.id/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarjatengProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\'(http\:\/\/www\.kabarjateng\.co\.id\/[0-9]+\/[0-9]+\/[^\']+\.html)\'/",
					$currentPage,
					$n
				)
			) {
