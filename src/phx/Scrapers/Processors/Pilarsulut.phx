 ...
 * @version 0.0.1
 */
final class Pilarsulut implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.pilarsulut.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/rel=\"author\">(.*?)<\/a>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Pilarsulut"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        //     $author = trim(html_entity_decode(strip_tags("Pilarsulut"), ENT_QUOTES, "UTF-8"));
	...
	{
		if( preg_match("/<div class=\"entry-content clearfix\">([\s|\S]*?)<div class=\"mom-share-post\">/", $string, $m) ) {
			$content = $m[1];
