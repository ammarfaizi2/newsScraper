...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Sulutdaily as SulutdailyProcessor;
 ...
 * @version 0.0.1
 */
final class Sulutdaily extends NewsScraper
{

	const targetList = [
        "Sulawesi Utara" => "http://sulutdaily.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new SulutdailyProcessor($url, $this);
