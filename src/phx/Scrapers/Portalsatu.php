...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Portalsatu as PortalsatuProcessor;
 ...
 * @version 0.0.1
 */
final class Portalsatu extends NewsScraper
{

	const targetList = [
        "Nanggroe Aceh Darussalam" => "http://portalsatu.com/kanal/terbaru"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new PortalsatuProcessor($url, $this);
