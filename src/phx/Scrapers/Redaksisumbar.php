...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Redaksisumbar as RedaksisumbarProcessor;
 ...
 * @version 0.0.1
 */
final class Redaksisumbar extends NewsScraper
{

	const targetList = [
        "Sumatera Barat" => "https://redaksisumbar.com/category/daerah/"
	];

    private $host = "https://redaksisumbar.com/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RedaksisumbarProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/class=\"entry-title\"><a href=\"(.*?)\" title=/",
					$currentPage,
					$n
				)
			) {
