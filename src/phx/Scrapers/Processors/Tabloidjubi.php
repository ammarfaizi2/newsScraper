 ...
 * @version 0.0.1
 */
final class Tabloidjubi implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://tabloidjubi.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/Editor : ([^\<]+)<\/div>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Tabloidjubi"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
		...
		if ( preg_match("/<li><i class=\"fa fa-calendar\"><\/i> [^\,]+\, (.*?) \&mdash\; (.*?)<\/li>/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"detail-text\"([\s\S]*?)<div id=\"disqus_thread\"><\/div>/", $string, $m) ) {
			$content = $m[1];
