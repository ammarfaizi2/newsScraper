 ...
 * @version 0.0.1
 */
final class Sulutdaily implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://sulutdaily.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"entry-title-single\">(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/Reporter \: <a href=\'[^\']+\'>([^\<]+)<\/a>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Sulutdaily"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
	...
	{
		if( preg_match("/<div class=\"entry-content\">([\s|\S]*?)<div class=\"sharedaddy/", $string, $m) ) {
			$content = $m[1];
