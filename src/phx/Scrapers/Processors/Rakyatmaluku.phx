 ...
 * @version 0.0.1
 */
final class Rakyatmaluku implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://rakyatmaluku.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"post-title entry-title left\" itemprop=\"headline\">(.*?)<\/h1>/", $l["out"], $m) ) {
         ...
         * 
         */
        if ( preg_match("/<strong>Oleh: ([^\<]+)<\/strong>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        } else {
            $author = trim(html_entity_decode(strip_tags("Rakyatmaluku"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        ...
        if ( preg_match("/datePublished\" datetime=\"([^\"]+)\"/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/\/section><\!--social-sharing-top-->([\s\S]*?)<div class=\"post-views/", $string, $m) ) {
			$content = $m[1];
