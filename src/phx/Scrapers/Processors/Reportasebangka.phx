 ...
 * @version 0.0.1
 */
final class Reportasebangka implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://reportasebangka.com/";
	...
	public function run(): bool
	{
         ...
         * Get Images
         */
        if ( preg_match("/<span class=\"image-caption\"> <img src=\"(.*?)\" alt=/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/<div class=\"calendar-date\"> (.*?) <\/div>/", $l["out"], $m) ) {
            $time   = $m[1];
            $exp    = explode(' ', $time);
            $date   = date('Y');
            $detail_time = "00:00:00";
            if ( preg_match("/color\:\#900\;\">(.*?)<\/font>/", $l["out"], $n) ) {
                $date = $n[1];
            }
            // <font>19:31 WIB</font>
            if ( preg_match("/<font>(.*?) WIB<\/font>/", $l["out"], $n) ) {
                $detail_time = $n[1] . ":00";
            }
            ...
            if( count($exp) > 1 ) {
                ...
                if( isset($this->months[$sub_3]) ) {
                    ...
                    $time       = $date . '/' . $real_mon . '/' . $exp[0] . ' ' . $detail_time;
                }
            }
	...
	{
		if( preg_match("/<div class=\"shortcode-content\"> (.*?) <\!-- END \.shortcode-content --> <\/div>/", $string, $m) ) {
            $content = $m[1];
