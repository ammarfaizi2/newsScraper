 ...
 * @version 0.0.1
 */
final class Kliksamarinda implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.kliksamarinda.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h2 class=\'post-title\'>(.*?)<\/h2>/", $l["out"], $m) ) {
         ...
         * Get Images
         */
        if ( preg_match("/<div class=\'img-float\'><img src=\'([^\']+)\'/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/<div class=\'post-meta\'>[a-zA-Z]+\, (.*?) WIB/", $l["out"], $m) ) {

            $time   = $m[1];

            $time   = str_replace('Jam:', '', $time);
	...
	{
		if( preg_match("/id=\'container-blog-post\'([\s|\S]*?)<div style=\'color\:\#999\;/", $string, $m) ) {
			$content = $m[1];
