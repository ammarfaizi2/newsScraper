...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Papuanews as PapuanewsProcessor;
 ...
 * @version 0.0.1
 */
final class Papuanews extends NewsScraper
{

	const targetList = [
        "Papua Barat" => "https://papuanews.id/tag/papua-barat/",
        "Papua" => "https://papuanews.id/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new PapuanewsProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/papuanews\.id\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
