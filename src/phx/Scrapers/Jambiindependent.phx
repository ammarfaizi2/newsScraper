...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Jambiindependent as JambiindependentProcessor;
 ...
 * @version 0.0.1
 */
final class Jambiindependent extends NewsScraper
{

	const targetList = [
        "Jambi" => "http://www.jambi-independent.co.id/indeks"
	];

    private $host = "http://www.jambi-independent.co.id/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new JambiindependentProcessor($url, $this);
