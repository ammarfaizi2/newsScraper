...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Harianhaluan as HarianhaluanProcessor;
 ...
 * @version 0.0.1
 */
final class Harianhaluan extends NewsScraper
{

	const targetList = [
        "Sumatera Barat" => "https://www.harianhaluan.com/news/indeks"
	];

    private $host = "https://www.harianhaluan.com/news/indeks";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new HarianhaluanProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/<h5 class=\"l_blue2_detik bold\">[^<]+<a href=\"(.*?)\"/",
					$currentPage,
					$n
				)
			) {
