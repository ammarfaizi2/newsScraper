...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Inradiofm as InradiofmProcessor;
 ...
 * @version 0.0.1
 */
final class Inradiofm extends NewsScraper
{

	const targetList = [
        "Bangka Belitung" => "https://www.inradiofm.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new InradiofmProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/a href=\'(https\:\/\/www\.inradiofm.com\/[0-9]+\/[0-9]+\/.+\.html)\'/",
					$currentPage,
					$n
				)
			) {
