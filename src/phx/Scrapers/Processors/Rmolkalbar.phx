 ...
 * @version 0.0.1
 */
final class Rmolkalbar implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
        ...
        // Get date and time
        // if ( preg_match("/\"datePublished\">(.*?)<\/span>/", $l["out"], $m) ) {
         ...
         * Get post type
         */
        if( preg_match("/name=\"og\:type\" content=\"(.*?)\" \/>/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"post newsContent\">([\s|\S]*?)<div class=\"visible-xs/", $string, $m) ) {
			$content = $m[1];
