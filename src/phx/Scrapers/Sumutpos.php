...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Sumutpos as SumutposProcessor;
 ...
 * @version 0.0.1
 */
final class Sumutpos extends NewsScraper
{

	const targetList = [
        "Sumatera Utara" => "https://sumutpos.co/rubrik/daerah/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new SumutposProcessor($url, $this);
