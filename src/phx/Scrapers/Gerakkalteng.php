...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Gerakkalteng as GerakkaltengProcessor;
 ...
 * @version 0.0.1
 */
final class Gerakkalteng extends NewsScraper
{

	const targetList = [
        "Kalimantan Tengah" => "https://gerakkalteng.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new GerakkaltengProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/gerakkalteng\.com\/[0-9]+\/[0-9]+\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
