 ...
 * @version 0.0.1
 */
final class Fokusjambi implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<div class=\"tagline-top putih[^>]+>[\s|\t|\n]+(.*?)[\s|\t|\n]+<\/div>/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/<span>[\S]+\,[\s]*(.*?)[\s]*WIB/", $l["out"], $m) ) {
            $time = $m[1];
	...
	{
		// if( preg_match("/contentberita[^>]+>([\s|\S]+)<div class=\"bancont right\">/", $string, $m) ) {
        //     $content = $m[1];
