...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Rmolbengkulu as RmolbengkuluProcessor;
 ...
 * @version 0.0.1
 */
final class Rmolbengkulu extends NewsScraper
{

	const targetList = [
        "Bengkulu" => "http://www.rmolbengkulu.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RmolbengkuluProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/www\.rmolbengkulu\.com\/read\/[0-9]+\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
