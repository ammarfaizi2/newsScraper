...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Indonesiaberita as IndonesiaberitaProcessor;
 ...
 * @version 0.0.1
 */
final class Indonesiaberita extends NewsScraper
{

	const targetList = [
        "Maluku Utara" => "https://www.indonesiaberita.com/berita-maluku-utara/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new IndonesiaberitaProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.indonesiaberita\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
