 ...
 * @version 0.0.1
 */
final class Kabarjawatimur implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.kabarjawatimur.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/og\:image\" content=\"([^\"]+)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<figcaption itemprop=\"caption description\">(.*?)<\/figcaption/", $l["out"], $n) ) {
        ...
        // Get date and time
        // if ( preg_match("/og\:url\' content=\'http\:\/\/surabaya\.bisnis\.com\/read\/([0-9]+)\/[0-9]+\//", $l["out"], $m) ) {
        ...
        //             if( preg_match("/[0-9]+ \| (.*?) WIB<\/span>/", $l["out"], $n) ) {
        //                 $detail_time = $n[1] . ':00';
        //             }
        ...
        //     $time   = $year . '/' . $mon . '/' . $day . ' ' . $detail_time;
            
        //     $this->datetime = $time;
		// 	icelog("Got time: " . $time);
        // }

        if ( preg_match("/published_time\" content=\"([^\"]+)\+[0-9\:]+\">/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"content-inner([\s|\S]*?)<div class=\"sharedaddy/", $string, $m) ) {
			$content = $m[1];
