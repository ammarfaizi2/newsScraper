...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Minangkabaunews as MinangkabaunewsProcessor;
 ...
 * @version 0.0.1
 */
final class Minangkabaunews extends NewsScraper
{

	const targetList = [
        "Sumatera Barat" => "https://minangkabaunews.com/index-berita-minangkabaunews.html"
	];

    private $host = "https://minangkabaunews.com/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new MinangkabaunewsProcessor($this->host . $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/<span class\=judul\_dlm\_kategori><a href\=([\S]+)\><b/",
					$currentPage,
					$n
				)
			) {
