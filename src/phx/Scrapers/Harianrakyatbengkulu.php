...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Harianrakyatbengkulu as HarianrakyatbengkuluProcessor;
 ...
 * @version 0.0.1
 */
final class Harianrakyatbengkulu extends NewsScraper
{

	const targetList = [
        "Bengkulu" => "http://harianrakyatbengkulu.com/ver3/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new HarianrakyatbengkuluProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/harianrakyatbengkulu\.com\/ver3\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
