 ...
 * @version 0.0.1
 */
final class Tajuktimur implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.tajuktimur.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"entry-title\">(.*?)<\/h1><\/div>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/Rep: ([^\<]+)[\s\t\n]+<\/p>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Tajuktimur"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
	...
	{
		if( preg_match("/<div class=\"entry-content\">([\s\S]*?)<div class='yuzo_related/", $string, $m) ) {
			$content = $m[1];
