...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Sulbarkita as SulbarkitaProcessor;
 ...
 * @version 0.0.1
 */
final class Sulbarkita extends NewsScraper
{

	const targetList = [
        "Sulawesi Barat" => "http://sulbarkita.com/news-index.html",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new SulbarkitaProcessor($this->host . $url, $this);
