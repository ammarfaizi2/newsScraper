 ...
 * @version 0.0.1
 */
final class Beritamalutco implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://beritamalut.co/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"jeg_post_title\">(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/Rep: ([^\<]+)[\s\t\n]+<\/p>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Beritamalutco"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
         ...
         * Get Images
         */
        if ( preg_match("/name=\"og\:image\"[\s]+content=\"([^\"]+)\" \/>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/name=\"article:published_time\" content=\"([^\+]+)\+/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"content-inner\">([\s\S]*?)<div class=\"jeg_share_bottom_container\">/", $string, $m) ) {
			$content = $m[1];
