...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Vifapost as VifapostProcessor;
 ...
 * @version 0.0.1
 */
final class Vifapost extends NewsScraper
{

	const targetList = [
        "Gorontalo" => "https://vifapost.com/category/gorontalo/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new VifapostProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/vifapost\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
