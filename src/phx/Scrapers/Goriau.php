...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Goriau as GoriauProcessor;
 ...
 * @version 0.0.1
 */
final class Goriau extends NewsScraper
{

	const targetList = [
        "Riau" => "https://www.goriau.com/berita/riau/index.html"
	];

    private $host = "https://www.goriau.com";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new GoriauProcessor($this->host . $url, $this);
