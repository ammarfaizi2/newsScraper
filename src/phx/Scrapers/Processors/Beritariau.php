 ...
 * @version 0.0.1
 */
final class Beritariau implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://beritariau.com/";
	...
	public function run(): bool
	{
         ...
         * Get Authors
         */
        if ( preg_match("/Penulis \: <b>(.*?)<\/b>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)[\s|\t|\n]*\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<p class=\'ket_gambar\'>(.*?)<\/p><\/div><p>/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/[\S]+\, (.*?)  WIB/", $l["out"], $m) ) {

            $time = $m[1];
            $exp = explode(' ', $time);
            $mon = $exp[1];
            $lower_mon = strtolower($mon);
            $sub_3 = substr($lower_mon, 0, 3);

            if( isset($this->months[$sub_3]) ) {
                $real_mon = $this->months[$sub_3];
                $real_mon = sprintf("%02d", $real_mon);

                $time = $exp[2] . '/' . $real_mon . '/' . $exp[0] . ' ' . $exp[4] . ':00';
            }
	...
	{
		if( preg_match("/<div class=\'garis_merah\'><\/div>([\s|\S]*?)<div class=\'kotak_head\'/", $string, $m) ) {
            $content = preg_replace("/<!--[\s]*(.*?)[\s]*-->/", "", $m[1]);
            $content = preg_replace("/<script[^>]+>(.*?)<\/script>/", "", $content);
            $content = preg_replace("/<script>[\s|\t|\n]*(.*?)[\s|\t|\n]*<\/script>/", "", $content);
            if ( preg_match("/\[(.*?)\]/", $content, $n) ) {
                $author = trim(html_entity_decode($n[1], ENT_QUOTES, "UTF-8"));
                $this->authors[] = $author;
                icelog("Got author: " . $author);
            }
			if( preg_match_all("/<img(.*?)\/>/", $content, $n) ) {
				foreach($n[1] AS $k => $v) {
					if ( preg_match("/src=\"(.*?)\"/", $v, $o) ) {

						$img = trim(html_entity_decode( $this->host . $o[1], ENT_QUOTES, "UTF-8"));
						icelog("Got image: " . $img);
						$des = "";

						if( preg_match("/alt=\"(.*?)\"/", $v, $o) ) {
							$des = trim(html_entity_decode($m[1], ENT_QUOTES, "UTF-8"));
							icelog("Got description: " . $des);
						}
            ...
            $this->content .= trim(html_entity_decode(strip_tags($content), ENT_QUOTES, "UTF-8"));
            icelog("Got content");
        }
