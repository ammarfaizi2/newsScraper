...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Infojogja as InfojogjaProcessor;
 ...
 * @version 0.0.1
 */
final class Infojogja extends NewsScraper
{

	const targetList = [
        "Daerah Istimewa Yogyakarta" => "http://www.info-jogja.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new InfojogjaProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\'(http\:\/\/www\.info-jogja\.com\/[0-9]+\/[0-9]+\/[^\']+\.html)\'/",
					$currentPage,
					$n
				)
			) {
