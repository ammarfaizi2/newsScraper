...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarjabar as KabarjabarProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarjabar extends NewsScraper
{

	const targetList = [
        "Jawa Barat" => "http://kabarjabar.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarjabarProcessor($url, $this);
