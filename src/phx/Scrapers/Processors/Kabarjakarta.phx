 ...
 * @version 0.0.1
 */
final class Kabarjakarta implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.kabarjakarta.com";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<div class=\"col-md-9 post-box\">[\s\t\n]+<h1>(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/<b>Penulis \: <\/b>(.*?)<\/li>/", $l["out"], $o) ) {
         ...
         * Get Images
         */
        if ( preg_match("/<div class=\"post-thumb\">(.*?)<\/small><\/div>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<img src=\"([^\"]+)\"/", $i, $n) ) {
                $img = trim(html_entity_decode($n[1], ENT_QUOTES, "UTF-8"));
                icelog("Got image: " . $this->host . $img);
            }

            if( preg_match("/<small>(.*?)/", $i, $n) ) {
        ...
        // Get date and time
        if ( preg_match("/<b>Terbit \:<\/b> (.*?)<\/li>/", $l["out"], $m) ) {
            ...
            if( count($exp) > 1 AND count($exp) <= 4 ) {
                $time = $exp[2] . "/" . $exp[1] . "/" . $exp[0] . " " . $exp[3] . ":00";
            }
	...
	{
		if( preg_match("/<div class=\"col-md-9 post-inner\">([\s|\S]*?)<div class=\"footer-social\">/", $string, $m) ) {
            $content = $m[1];
