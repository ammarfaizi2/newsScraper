 ...
 * @version 0.0.1
 */
final class Kabarjateng implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.kabarjateng.co.id/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h3 class=\'post-title entry-title\' itemprop=\'name\'>[\s\t\n]+(.*?)[\s\t\n]+<\/h3>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/title=\'author profile\'>[\s\t\n]+<span itemprop=\'name\'>(.*?)<\/span>/", $l["out"], $o) ) {
	...
	{
		if( preg_match("/itemprop=\'description articleBody\'>([\s|\S]*?)<div class=\'post-footer\'>/", $string, $m) ) {
			$content = $m[1];
