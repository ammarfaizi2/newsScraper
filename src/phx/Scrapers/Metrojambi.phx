...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Metrojambi as MetrojambiProcessor;
 ...
 * @version 0.0.1
 */
final class Metrojambi extends NewsScraper
{

	const targetList = [
        "Jambi" => "https://metrojambi.com/indeks"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new MetrojambiProcessor($url, $this);
