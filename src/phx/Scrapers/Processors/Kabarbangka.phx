 ...
 * @version 0.0.1
 */
final class Kabarbangka implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://kabarbangka.com/";
	...
	public function run(): bool
	{
        ...
        // Get date and time
        if ( preg_match("/class=\"updated\">(.*?)<\/span>/", $l["out"], $m) ) {
            $time   = $m[1];
	...
	{
		if( preg_match("/<div class=\"entry\">([\s|\S]+)<\/div><\!-- \.entry \/-->/", $string, $m) ) {
            $content = $m[1];
