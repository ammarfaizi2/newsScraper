...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kotajogja as KotajogjaProcessor;
 ...
 * @version 0.0.1
 */
final class Kotajogja extends NewsScraper
{

	const targetList = [
        "Daerah Istimewa Yogyakarta" => "https://www.kotajogja.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KotajogjaProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.kotajogja\.com\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
