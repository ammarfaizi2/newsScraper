...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritajambi as BeritajambiProcessor;
 ...
 * @version 0.0.1
 */
final class Beritajambi extends NewsScraper
{

	const targetList = [
        "Jambi" => "http://beritajambi.co/"
	];

    private $host = "http://beritajambi.co/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritajambiProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/\"(http\:\/\/beritajambi\.co\/read\/[0-9a-zA-Z\-\/]+)\"/",
					$currentPage,
					$n
				)
			) {
