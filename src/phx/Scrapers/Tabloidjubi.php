...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Tabloidjubi as TabloidjubiProcessor;
 ...
 * @version 0.0.1
 */
final class Tabloidjubi extends NewsScraper
{

	const targetList = [
        "Papua" => "http://tabloidjubi.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new TabloidjubiProcessor( $this->host . $url, $this);
