...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Sulutpost as SulutpostProcessor;
 ...
 * @version 0.0.1
 */
final class Sulutpost extends NewsScraper
{

	const targetList = [
        "Sulawesi Utara" => "https://sulutpost.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new SulutpostProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/sulutpost\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
