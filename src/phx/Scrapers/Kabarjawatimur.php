...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarjawatimur as KabarjawatimurProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarjawatimur extends NewsScraper
{

	const targetList = [
        "Jawa Timur" => "https://www.kabarjawatimur.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarjawatimurProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.kabarjawatimur\.com\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
