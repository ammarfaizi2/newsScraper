...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Pikiranrakyat as PikiranrakyatProcessor;
 ...
 * @version 0.0.1
 */
final class Pikiranrakyat extends NewsScraper
{

	const targetList = [
        "Jawa Barat" => "http://www.pikiran-rakyat.com/jawa-barat"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new PikiranrakyatProcessor($this->host . $url, $this);
