 ...
 * @version 0.0.1
 */
final class Korankaltim implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.korankaltim.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"font-4x marg-ver-none[^\>]+\">(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/Reporter \: <a href=\'[^\']+\'>([^\<]+)<\/a>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Korankaltim"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        ...
        // Get date and time
        if ( preg_match("/Posted\: (.*?)<\/div>/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"panel-body protect-text\"([\s|\S]*?)class=\"blue2 bdrblue\">berita<\/span> /", $string, $m) ) {
			$content = $m[1];
