 ...
 * @version 0.0.1
 */
final class Tribratanewspapuabarat implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://tribratanewspapuabarat.com/";
	...
	public function run(): bool
	{
         ...
         * 
         */
        if ( preg_match("/Editor : ([^\<]+)<\/div>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        } else {
            $author = trim(html_entity_decode(strip_tags("RedakturTribata"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        
        //     $author = trim(html_entity_decode(strip_tags("Tribratanewspapuabarat"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
	...
	{
		if( preg_match("/<div class=\"konten-berita\">([\s\S]*?)<div class=\"col-sm-16 article-sharer/", $string, $m) ) {
			$content = $m[1];
