...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritamalutcom as BeritamalutcomProcessor;
 ...
 * @version 0.0.1
 */
final class Beritamalutcom extends NewsScraper
{

	const targetList = [
        "Maluku Utara" => "https://beritamalut.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritamalutcomProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/beritamalut\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
