...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritajatim as BeritajatimProcessor;
 ...
 * @version 0.0.1
 */
final class Beritajatim extends NewsScraper
{

	const targetList = [
        "Jawa Timur" => "http://beritajatim.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritajatimProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/beritajatim\.com\/[^\"]+\.html)\"/",
					$currentPage,
					$n
				)
			) {
