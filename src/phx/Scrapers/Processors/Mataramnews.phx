 ...
 * @version 0.0.1
 */
final class Mataramnews implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://mataramnews.co.id";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1>([\s\S]+)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/og:image\" content=\"([^\"]+)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<span class=\"itemImageCaption\">(.*?)<\/span>/", $l["out"], $n) ) {
        ...
        if ( preg_match("/<time datetime=\"(.*?)\+/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"itemBody\">([\s|\S]*?)<div id=\"gko_me\">/", $string, $m) ) {
			$content = $m[1];
