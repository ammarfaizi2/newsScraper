...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Batampos as BatamposProcessor;
 ...
 * @version 0.0.1
 */
final class Batampos extends NewsScraper
{

	const targetList = [
        "Kepulauan Riau" => "https://batampos.co.id/kepri/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BatamposProcessor($url, $this);
