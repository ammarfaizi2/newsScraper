 ...
 * @version 0.0.1
 */
final class Kabarsulteng implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://kabarsulteng.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h2 class=\"title\">(.*?)<\/h2>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/rel=\"author\">([^\<]+)</", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Kabarsulteng"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        //     $author = trim(html_entity_decode(strip_tags("Kabarsulteng"), ENT_QUOTES, "UTF-8"));
	...
	{
		if( preg_match("/<div class=\"entry clearfix\">([\s|\S]*?)<div id=\"respond\"/", $string, $m) ) {
			$content = $m[1];
