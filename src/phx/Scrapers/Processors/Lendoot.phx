 ...
 * @version 0.0.1
 */
final class Lendoot implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
		...
		if ( preg_match("/<h1 class=\"entry-title\">(.*?)<\/h1>/", $l["out"], $m) ) {
			$this->title = trim(html_entity_decode( strip_tags( str_replace(" - Sijori Kepri", "", $m[1]) ), ENT_QUOTES, "UTF-8"));
         ...
         * Get Images
         */
        if ( preg_match("/td-modal-image\" src=\"(.*?)\"/", $l["out"], $m) ) {
        ...
        // Get Tags 
        if ( preg_match("/<div class=\"td-post-source-tags\">[\s|\t|\n]+(.*?)<\/li><\/ul>/", $l["out"], $m) ) {
            $contentTags = $m[1];
