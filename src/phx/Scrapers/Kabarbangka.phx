...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarbangka as KabarbangkaProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarbangka extends NewsScraper
{

	const targetList = [
        "Bangka Belitung" => "http://kabarbangka.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarbangkaProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/<a href=\"(http\:\/\/kabarbangka\.com\/[0-9]+\/[0-9]+\/.+\/)\"/",
					$currentPage,
					$n
				)
			) {
