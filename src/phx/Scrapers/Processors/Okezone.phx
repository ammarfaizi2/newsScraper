 ...
 * @version 0.0.1
 */
final class Okezone implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.okezone.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<div class=\"namerep\">[\s\t\n]+(.*?)\,[\s\t\n]+<span>/", $l["out"], $o) ) {
        ...
        //     $author = trim(html_entity_decode(strip_tags("Okezone"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
        ...
        if ( preg_match("/og\:image\"[\s]+content=\"([^\"]+)\">/", $l["out"], $m) ) {
            ...
            if( preg_match("/itemprop=\"image\">[\s\t\n]+<span>(.*?)<\/span>/", $l["out"], $n) ) {
        ...
        if( preg_match_all("/class=\"ga_Tag\">(.*?)<\/a>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
	...
	{
		if( preg_match("/<div id=\"contentx\" class=\"read\">([\s|\S]*?)<div class=\"detail-tag\">/", $string, $m) ) {
			$content = $m[1];
