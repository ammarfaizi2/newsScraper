...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritamalukuonline as BeritamalukuonlineProcessor;
 ...
 * @version 0.0.1
 */
final class Beritamalukuonline extends NewsScraper
{

	const targetList = [
        "Maluku Utara" => "http://www.beritamalukuonline.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritamalukuonlineProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href='(http\:\/\/www\.beritamalukuonline\.com\/[0-9]+\/[0-9]+\/[^\']+\.html)'/",
					$currentPage,
					$n
				)
			) {
