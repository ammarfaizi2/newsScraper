...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabartangsel as KabartangselProcessor;
 ...
 * @version 0.0.1
 */
final class Kabartangsel extends NewsScraper
{

	const targetList = [
        "Banten" => "https://kabartangsel.com/tag/kabar-banten/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabartangselProcessor( $url, $this);
