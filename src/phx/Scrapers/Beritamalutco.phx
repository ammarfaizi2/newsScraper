...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritamalutco as BeritamalutcoProcessor;
 ...
 * @version 0.0.1
 */
final class Beritamalutco extends NewsScraper
{

	const targetList = [
        "Maluku Utara" => "https://beritamalut.co/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritamalutcoProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/beritamalut\.co\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
