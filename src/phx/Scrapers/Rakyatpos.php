...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Rakyatpos as RakyatposProcessor;
 ...
 * @version 0.0.1
 */
final class Rakyatpos extends NewsScraper
{

	const targetList = [
        "Bangka Belitung" => "http://www.rakyatpos.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RakyatposProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/www\.rakyatpos\.com\/[a-zA-Z\-]+\.html\/)\"/",
					$currentPage,
					$n
				)
			) {
