 ...
 * @version 0.0.1
 */
final class Pasificpos implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.pasificpos.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h2 class=\"itemTitle text-uppercase\">([\s\S]*?)<\/h2>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/<a rel=\"author\" href=\"[^\"]+\">([^\<]+)<\/a>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Pasificpos"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
	...
	{
		if( preg_match("/<div class=\"itemIntroText\">([\s\S]*?)<div class=\"itemContentFooter\">/", $string, $m) ) {
			$content = $m[1];
