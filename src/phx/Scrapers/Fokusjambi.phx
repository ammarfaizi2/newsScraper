...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Fokusjambi as FokusjambiProcessor;
 ...
 * @version 0.0.1
 */
final class Fokusjambi extends NewsScraper
{

	const targetList = [
        "Jambi" => "https://fokusjambi.com/"
	];

    private $host = "https://fokusjambi.com/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new FokusjambiProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/\"(https:\/\/fokusjambi\.com\/post\/detail\/[\S]+\.html)\"/",
					$currentPage,
					$n
				)
			) {
