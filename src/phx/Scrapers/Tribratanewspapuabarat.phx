...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Tribratanewspapuabarat as TribratanewspapuabaratProcessor;
 ...
 * @version 0.0.1
 */
final class Tribratanewspapuabarat extends NewsScraper
{

	const targetList = [
        "Papua Barat" => "http://tribratanewspapuabarat.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new TribratanewspapuabaratProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/tribratanewspapuabarat\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
