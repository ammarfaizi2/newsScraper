...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Infosumbar as InfosumbarProcessor;
 ...
 * @version 0.0.1
 */
final class Infosumbar extends NewsScraper
{

	const targetList = [
        "Sumatera Barat" => "https://www.infosumbar.net/kanal/berita/berita-sumbar/"
	];

    private $host = "https://www.infosumbar.net/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new InfosumbarProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/<a href=\"(https\:\/\/www\.infosumbar\.net\/berita\/.*?)\"/",
					$currentPage,
					$n
				)
			) {
