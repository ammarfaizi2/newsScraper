 ...
 * @version 0.0.1
 */
final class Beritapapua implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://berita-papua.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/class=\"post-author-name\"><b>([^\<]+)<\/b>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Beritapapua"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        ...
        if ( preg_match("/date published[ updated]*\" datetime=\"([^\+]+)\+/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"entry-content\">([\s\S]*?)<footer class=\"entry-footer\">/", $string, $m) ) {
			$content = $m[1];
