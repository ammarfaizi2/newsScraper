 ...
 * @version 0.0.1
 */
final class Fajaronline implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://fajaronline.co.id/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h2>(.*?)<\/h2>[\s\t\n]+<\/div>[\s\t\n]+<div class=\"main-article-content\">/", $l["out"], $m) ) {
        ...
        if ( preg_match("/Author \: (.*?)<\/p>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        } else {
            $author = trim(html_entity_decode(strip_tags("Fajaronline"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        //     $author = trim(html_entity_decode(strip_tags("Fajaronline"), ENT_QUOTES, "UTF-8"));
	...
	{
		if( preg_match("/<div class=\"breaking-news\">([\s|\S]*?)<p style=\"font-size\:12px\;\">/", $string, $m) ) {
			$content = $m[1];
