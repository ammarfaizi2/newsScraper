 ...
 * @version 0.0.1
 */
final class Beritasulteng implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.beritasulteng.id/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"name post-title entry-title\" itemprop=\"name\">(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/<p>Laporan\,  (.*?)<\/p>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        
        //     $author = trim(html_entity_decode(strip_tags("Beritasulteng"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
	...
	{
		if( preg_match("/<div class=\"entry\">([\s|\S]*?)<span style=\"display\:none\"/", $string, $m) ) {
			$content = $m[1];
