 ...
 * @version 0.0.1
 */
final class Suarantb implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.suarantb.com/";
	...
	public function run(): bool
	{
        ...
        // if ( preg_match("/Editor \: <span itemprop=\"editor\">([^\<]+)<\/span>/", $l["out"], $o) ) {
        //     $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
        ...
        if ( preg_match("/og:image\" content=\"([^\"]+)\" \/><link/", $l["out"], $m) ) {
            ...
            $img = trim(html_entity_decode( 'http:' . $i, ENT_QUOTES, "UTF-8"));
        ...
        if( preg_match_all("/<li><a href=\"https\:\/\/www\.suarantb\.com\/tag\/[^\"]+\">(.*?)<\/a><\/li>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
	...
	private function doContent(string $string)
	{
		if( preg_match("/<div class=\"td-post-content\">([\s|\S]*?)<footer>/", $string, $m) ) {
			...
			if( preg_match_all("/<p[^\>]*>(.*?)<\/p>/", $content, $o) ) {
