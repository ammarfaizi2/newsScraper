 ...
 * @version 0.0.1
 */
final class Hargasumut implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Authors
         */
        if ( preg_match("/Sumber \: (.*?)<\/p>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)\"\/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<p style=text\-align\:justify>(.*?)<br \/>/", $l["out"], $m) ) {
	...
	{
		if ( preg_match("/class=\"max-width pbottom05\">[\s|\n|\t]+<\/center>([\s|\S]+)<\!\-\- AddThis/", $string, $m) ) {
            $rawContent = $m[1];
