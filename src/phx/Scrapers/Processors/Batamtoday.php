 ...
 * @version 0.0.1
 */
final class Batamtoday implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<\/span>Oleh \:(.*?)<\/div>/", $l["out"], $o) ) {
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<font size=\"-2\">(.*?)<\/font>/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/PublishedDate\" content=\"(.*?)\"/", $l["out"], $m) ) {
