...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Infobanten as InfobantenProcessor;
 ...
 * @version 0.0.1
 */
final class Infobanten extends NewsScraper
{

	const targetList = [
        "Banten" => "http://www.infobanten.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new InfobantenProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				...
				preg_match_all(
					"/href=\"(http\:\/\/www\.infobanten\.com\/[^\/]+-banten\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
