...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritasampit as BeritasampitProcessor;
 ...
 * @version 0.0.1
 */
final class Beritasampit extends NewsScraper
{

	const targetList = [
        "Kalimantan Tengah" => "https://beritasampit.co.id/category/kalteng/",
	];

    private $host = "https://beritasampit.co.id/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritasampitProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/beritasampit\.co\.id\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
