...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Pasificpos as PasificposProcessor;
 ...
 * @version 0.0.1
 */
final class Pasificpos extends NewsScraper
{

	const targetList = [
        "Papua Barat" => "https://www.pasificpos.com/papua-barat",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new PasificposProcessor( $this->host . $url, $this);
