 ...
 * @version 0.0.1
 */
final class Indonesiaberita implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.indonesiaberita.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"single-post-title\">([\s\S]*?)<\/h1>/", $l["out"], $m) ) {
         ...
         * 
         */
        if ( preg_match("/class=\"post-author-name\"><b>([^\<]+)<\/b>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        } else {
            $author = trim(html_entity_decode(strip_tags("Indonesiaberita"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        ...
        if ( preg_match("/og\:image\" content=\"([^\"]+)\"\/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/og\:image:alt\" content=\"([^\"]+)\"\/>/", $l["out"], $n) ) {
	...
	{
		if( preg_match("/<div class=\"entry-content clearfix single-post-content\">([\s\S]*?)<div class=\"entry-terms post-tags clearfix\">/", $string, $m) ) {
			$content = $m[1];
