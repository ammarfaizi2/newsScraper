 ...
 * @version 0.0.1
 */
final class Infosumbar implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
		...
		if ( preg_match("/og\:title\" content=\"(.*?)\" \/>/", $l["out"], $m) ) {
			$this->title = trim(html_entity_decode( str_replace(' - infoSumbar', '', $m[1]), ENT_QUOTES, "UTF-8"));
         ...
         * Get Authors
         */
        if ( preg_match("/rel=\"author\">(.*?)<\/a>/", $l["out"], $m) ) {
         ...
         * Get Images
         */
        if ( preg_match("/og\:image\" content=\"(.*?)\" \/><meta name/", $l["out"], $m) ) {
	...
	{
		if ( preg_match("/<\/section><p>(.*?)<\/p><div/", $string, $m) ) {
            $rawContent = $m[1];
