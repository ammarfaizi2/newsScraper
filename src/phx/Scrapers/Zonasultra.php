...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Zonasultra as ZonasultraProcessor;
 ...
 * @version 0.0.1
 */
final class Zonasultra extends NewsScraper
{

	const targetList = [
        "Sulawesi Tenggara" => "https://zonasultra.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new ZonasultraProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/zonasultra\.com\/[^\"]+\.html)\"/",
					$currentPage,
					$n
				)
			) {
