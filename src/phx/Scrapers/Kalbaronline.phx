...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kalbaronline as KalbaronlineProcessor;
 ...
 * @version 0.0.1
 */
final class Kalbaronline extends NewsScraper
{

	const targetList = [
        "Kalimantan Barat" => "https://www.kalbaronline.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KalbaronlineProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.kalbaronline\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
