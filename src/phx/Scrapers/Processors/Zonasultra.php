 ...
 * @version 0.0.1
 */
final class Zonasultra implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://zonasultra.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<h5>Reporter\: (.*?)<br/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        if ( preg_match("/Editor : (.*?)<\/h5>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        
        //     $author = trim(html_entity_decode(strip_tags("Zonasultra"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
         ...
         * Get Images
         */
        if ( preg_match("/og\:image\"[\s]+content=\"([^\"]+)\" \/><link/", $l["out"], $m) ) {
