...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Jabarnews as JabarnewsProcessor;
 ...
 * @version 0.0.1
 */
final class Jabarnews extends NewsScraper
{

	const targetList = [
        "Jawa Barat" => "http://jabarnews.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new JabarnewsProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/jabarnews\.com\/[0-9]+\/[0-9]+\/[^\"]+\.html)\"/",
					$currentPage,
					$n
				)
			) {
