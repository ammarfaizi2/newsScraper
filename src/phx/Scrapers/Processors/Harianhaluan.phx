 ...
 * @version 0.0.1
 */
final class Harianhaluan implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
        ...
        // Get date and time
        if ( preg_match("/published_time\" content=\"[\S]+\, (.*?) \+/", $l["out"], $m) ) {
            $time = $m[1];
            $exp_time = explode(' ', $time);
            $mon = $exp_time[1];
            $lower_mon = strtolower($mon);
            if( isset($this->months[$lower_mon]) ) {
                $real_mon = sprintf("%02d", $this->months[$lower_mon]);
                $time = $exp_time[2] . '/' . $real_mon . '/' . $exp_time[0] . ' ' . $exp_time[3];
            }
        ...
        // Get Tags 
        if ( preg_match("/<strong>Kategori<\/strong><\/td>(.*?)<\/table>/", $l["out"], $m) ) {
            $contentTags = $m[1];
	...
	{
		if ( preg_match("/<div itemprop=\"articleBody\">([\s|\S]+)<\/div>[\s|\t|\n]+<div id=\"pagination\">/", $string, $m) ) {
            $rawContent = $m[1];
