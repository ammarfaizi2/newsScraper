...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Lendoot as LendootProcessor;
 ...
 * @version 0.0.1
 */
final class Lendoot extends NewsScraper
{

	const targetList = [
        "Kepulauan Riau" => "http://lendoot.com/category/berita-kepri/"
	];

    private $host = "http://lendoot.com";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new LendootProcessor($url, $this);
