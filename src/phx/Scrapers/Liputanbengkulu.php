...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Liputanbengkulu as LiputanbengkuluProcessor;
 ...
 * @version 0.0.1
 */
final class Liputanbengkulu extends NewsScraper
{

	const targetList = [
        "Bengkulu" => "http://liputanbengkulu.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new LiputanbengkuluProcessor($url, $this);
