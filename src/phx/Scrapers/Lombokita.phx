...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Lombokita as LombokitaProcessor;
 ...
 * @version 0.0.1
 */
final class Lombokita extends NewsScraper
{

	const targetList = [
        "Nusa Tenggara Barat" => "http://lombokita.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new LombokitaProcessor( $url, $this);
