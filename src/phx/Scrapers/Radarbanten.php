...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Radarbanten as RadarbantenProcessor;
 ...
 * @version 0.0.1
 */
final class Radarbanten extends NewsScraper
{

	const targetList = [
        "Banten" => "https://www.radarbanten.co.id/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RadarbantenProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.radarbanten\.co\.id\/[^\"]+)\" rel=\"bookmark\"/",
					$currentPage,
					$n
				)
			) {
