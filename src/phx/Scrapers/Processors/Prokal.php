 ...
 * @version 0.0.1
 */
final class Prokal implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<div style=\"font-family\: utopiareg\;[^\"]+\">(.*?)<\/div>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/og:image\" content=\"([^\"]+)\">/", $l["out"], $m) ) {
            ...
            if( preg_match("/<div class=\"text-muted\" style=\"padding\:10px 0\;\">(.*?)<\/div>/", $l["out"], $n) ) {
	...
	{
		if( preg_match("/<div id=\"bodytext\">([\s|\S]*?)<div style=\"margin-top\:10px\;\">/", $string, $m) ) {
			$content = $m[1];
