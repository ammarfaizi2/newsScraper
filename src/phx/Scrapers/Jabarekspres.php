...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Jabarekspres as JabarekspresProcessor;
 ...
 * @version 0.0.1
 */
final class Jabarekspres extends NewsScraper
{

	const targetList = [
        "Jawa Barat" => "http://jabarekspres.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new JabarekspresProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/jabarekspres\.com\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
