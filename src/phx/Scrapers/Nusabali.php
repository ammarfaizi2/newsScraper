...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Nusabali as NusabaliProcessor;
 ...
 * @version 0.0.1
 */
final class Nusabali extends NewsScraper
{

	const targetList = [
        "Bali" => "https://www.nusabali.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new NusabaliProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.nusabali\.com\/berita\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
