...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Hargasumut as HargasumutProcessor;
 ...
 * @version 0.0.1
 */
final class Hargasumut extends NewsScraper
{

	const targetList = [
        "Sumatera Utara" => "http://www.hargasumut.org/category/berita-sumut.html"
	];

    const host = "http://www.hargasumut.org";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new HargasumutProcessor(self::host . $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/<a href=\"http\:\/\/www\.hargasumut\.org(.*?)\"><img/",
					$currentPage,
					$n
				)
			) {
