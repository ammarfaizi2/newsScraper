...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Krjogja as KrjogjaProcessor;
 ...
 * @version 0.0.1
 */
final class Krjogja extends NewsScraper
{

	const targetList = [
        "Daerah Istimewa Yogyakarta" => "http://krjogja.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KrjogjaProcessor( $this->host . $url, $this);
