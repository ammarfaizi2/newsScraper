...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Voxntt as VoxnttProcessor;
 ...
 * @version 0.0.1
 */
final class Voxntt extends NewsScraper
{

	const targetList = [
        "Nusa Tenggara Timur" => "https://voxntt.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new VoxnttProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/voxntt\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
