 ...
 * @version 0.0.1
 */
final class Papuaus implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.papua.us/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h2 class='post-title entry-title' itemprop='name headline'>[\s\t\n]+(.*?)[\s\t\n]+<\/h2>/", $l["out"], $m) ) {
         ...
         * 
         */
        if ( preg_match("/<span itemprop='name'>([^<]+)<\/span>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        } else {
            $author = trim(html_entity_decode(strip_tags("Redaksi"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        
        //     $author = trim(html_entity_decode(strip_tags("Papuaus"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
         ...
         * Get Images
         */
        if ( preg_match("/content='([^\']+)' property='og:image'/", $l["out"], $m) ) {
            ...
            if( preg_match("/<div class=\"small-description\">[\s\t\n]+(.*?)[\s\t\n]+<\/div>/", $l["out"], $n) ) {
        ...
        // if ( preg_match("/<figure[^\>]*>([\s\S]*?)<\/figure>/", $l["out"], $m) ) {
        ...
        //     if( preg_match("/src=\"([^\"]+)\"/", $i, $n) ) {
        //         $img = trim(html_entity_decode( $n[1], ENT_QUOTES, "UTF-8"));
        //         icelog("Got image: " . $img);
        //     }
		...
		// if ( preg_match("/<div class=\"time\"><span class=\"ion-android-data icon\"><\/span>[^\,]+\, (.*?)<\/div>/", $l["out"], $m) ) {
        ...
        //     // $time   = $year . $mon . $date . $detail_time;
        ...
        if ( preg_match("/'datePublished' title='([^\+]+)\+/", $l["out"], $m) ) {
        ...
        // if( preg_match_all("/<h3>\#(.*?)<\/h3>/", $l["out"], $n) ) {
        //     foreach ($n[1] AS $key => $tag) {
        //         $this->tags[] = ucwords($tag);
        //         icelog("Got tag: " . ucwords($tag));
        //     }
         ...
         * Get post type
         */
        if( preg_match("/content='([^\']+)' property='og:type'/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/itemprop='description articleBody'>([\s\S]*?)<div class='post-labels'/", $string, $m) ) {
			$content = $m[1];
			...
			} else if( preg_match_all("/<div dir=\"auto\">([\s\S]*?)<\/div>/", $content, $o) ) {
				foreach ($o[1] as $key => $value) {
                    $next = $key + 1;
					// $value = str_replace(" Tinggalkan Komentar anda", "", $value);
					$this->content .= trim(html_entity_decode(strip_tags($value) . "\n", ENT_QUOTES, "UTF-8"));
            ...
            } else if( preg_match_all("/<div style=\"text-align: justify\;\">([\s\S]*?)<\/div>/", $content, $o) ) {
				foreach ($o[1] as $key => $value) {
                    $next = $key + 1;
					// $value = str_replace(" Tinggalkan Komentar anda", "", $value);
					$this->content .= trim(html_entity_decode(strip_tags($value) . "\n", ENT_QUOTES, "UTF-8"));
