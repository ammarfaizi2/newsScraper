...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Postkeadilan as PostkeadilanProcessor;
 ...
 * @version 0.0.1
 */
final class Postkeadilan extends NewsScraper
{

	const targetList = [
        "Jawa Barat" => "http://postkeadilan.com/category/kabar-jabar/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new PostkeadilanProcessor('http:' . $url, $this);
