...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Jambiekspres as JambiekspresProcessor;
 ...
 * @version 0.0.1
 */
final class Jambiekspres extends NewsScraper
{

	const targetList = [
        "Jambi" => "http://www.jambiekspres.co.id/"
	];

    private $host = "http://www.jambiekspres.co.id/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new JambiekspresProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/\"(http:\/\/www\.jambiekspres\.co\.id\/read\/[0-9\/]+[\S]+)\"/",
					$currentPage,
					$n
				)
			) {
