...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Infojatim as InfojatimProcessor;
 ...
 * @version 0.0.1
 */
final class Infojatim extends NewsScraper
{

	const targetList = [
        "Jawa Timur" => "http://www.infojatim.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new InfojatimProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\'(http\:\/\/www\.infojatim\.com\/[0-9]+\/[0-9]+\/[^\']+\.html)\'/",
					$currentPage,
					$n
				)
			) {
