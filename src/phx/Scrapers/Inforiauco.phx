...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Inforiauco as InforiaucoProcessor;
 ...
 * @version 0.0.1
 */
final class Inforiauco extends NewsScraper
{

	const targetList = [
        "Riau" => "https://www.inforiau.co/news/index"
	];

    private $host = "https://www.inforiau.co/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new InforiaucoProcessor($url, $this);
