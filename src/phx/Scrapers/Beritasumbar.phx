...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritasumbar as BeritasumbarProcessor;
 ...
 * @version 0.0.1
 */
final class Beritasumbar extends NewsScraper
{

	const targetList = [
        "Sumatera Barat" => "http://beritasumbar.com/BeritaSumbar/sumbar-hari-ini/"
	];

    private $host = "http://beritasumbar.com/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritasumbarProcessor($url, $this);
