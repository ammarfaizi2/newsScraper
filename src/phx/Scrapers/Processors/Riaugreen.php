 ...
 * @version 0.0.1
 */
final class Riaugreen implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Authors
         */
        if ( preg_match("/name=\"author\" content=\"(.*?)\" \/>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)\"\/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/class=\"post-thumb-desc\">(.*?)<\/div>/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/class=\"date-time\">[\S]+\, (.*?)<\/div>/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<\!--END TAG-->[\s|\t|\n]*<div class=isi>([\s|\S]*?)<\/div><div><\!-- Composite Start -->/", $string, $m) ) {
