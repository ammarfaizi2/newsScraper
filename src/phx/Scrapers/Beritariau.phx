...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritariau as BeritariauProcessor;
 ...
 * @version 0.0.1
 */
final class Beritariau extends NewsScraper
{

	const targetList = [
        "Riau" => "http://beritariau.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritariauProcessor($this->host . $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/<a href=(.*?)><span class=\'judulberita1\'>/",
					$currentPage,
					$n
				)
			) {
