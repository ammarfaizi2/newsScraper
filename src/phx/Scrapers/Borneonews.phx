...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Borneonews as BorneonewsProcessor;
 ...
 * @version 0.0.1
 */
final class Borneonews extends NewsScraper
{

	const targetList = [
        "Kalimantan Tengah" => "https://www.borneonews.co.id/",
	];

    private $host = "https://www.borneonews.co.id";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BorneonewsProcessor( $this->host . $url, $this);
