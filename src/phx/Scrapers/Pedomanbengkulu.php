...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Pedomanbengkulu as PedomanbengkuluProcessor;
 ...
 * @version 0.0.1
 */
final class Pedomanbengkulu extends NewsScraper
{

	const targetList = [
        "Bengkulu" => "http://pedomanbengkulu.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new PedomanbengkuluProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/pedomanbengkulu\.com\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
