...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Pilarsulut as PilarsulutProcessor;
 ...
 * @version 0.0.1
 */
final class Pilarsulut extends NewsScraper
{

	const targetList = [
        "Sulawesi Utara" => "https://www.pilarsulut.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new PilarsulutProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.pilarsulut\.com\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
