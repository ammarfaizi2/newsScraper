...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Modusaceh as ModusacehProcessor;
 ...
 * @version 0.0.1
 */
final class Modusaceh extends NewsScraper
{

	const targetList = [
        "Nanggroe Aceh Darussalam" => "http://modusaceh.co/news/index.html"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new ModusacehProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/itemprop=\"name\"> <a href=\"(.*?)\" rel=\"bookmark\">/",
					$currentPage,
					$n
				)
			) {
