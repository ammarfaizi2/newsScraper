...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Goaceh as GoacehProcessor;
 ...
 * @version 0.0.1
 */
final class Goaceh extends NewsScraper
{

	const targetList = [
        "Nanggroe Aceh Darussalam" => "https://www.goaceh.co/berita/aceh/"
	];
	 ...
	 * Go Aceh Site URL
	 */
    private $goAcehUrl = "https://www.goaceh.co";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new GoacehProcessor($url, $this);
		...
		do {
			...
			if(
				...
				array_walk($n[1], function ($url) use (&$urls) {
						$urls[] = trim(html_entity_decode($this->goAcehUrl . $url, ENT_QUOTES, "UTF-8"));
					});
			} else {
