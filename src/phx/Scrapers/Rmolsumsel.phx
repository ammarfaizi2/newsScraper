...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Rmolsumsel as RmolsumselProcessor;
 ...
 * @version 0.0.1
 */
final class Rmolsumsel extends NewsScraper
{

	const targetList = [
        "Sumatera Selatan" => "http://www.rmolsumsel.com/indeksberita.php"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RmolsumselProcessor($url, $this);
