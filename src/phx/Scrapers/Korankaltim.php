...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Korankaltim as KorankaltimProcessor;
 ...
 * @version 0.0.1
 */
final class Korankaltim extends NewsScraper
{

	const targetList = [
        "Kalimantan Timur" => "https://www.korankaltim.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KorankaltimProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.korankaltim\.com\/[^\/]+\/read\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
