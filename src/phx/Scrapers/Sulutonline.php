...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Sulutonline as SulutonlineProcessor;
 ...
 * @version 0.0.1
 */
final class Sulutonline extends NewsScraper
{

	const targetList = [
        "Sulawesi Utara" => "http://sulutonline.com/berita",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new SulutonlineProcessor($this->host . $url, $this);
