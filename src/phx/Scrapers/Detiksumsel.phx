...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Detiksumsel as DetiksumselProcessor;
 ...
 * @version 0.0.1
 */
final class Detiksumsel extends NewsScraper
{

	const targetList = [
        "Sumatera Selatan" => "https://www.detiksumsel.com/page/1"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new DetiksumselProcessor($url, $this);
