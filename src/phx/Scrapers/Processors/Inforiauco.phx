 ...
 * @version 0.0.1
 */
final class Inforiauco implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Authors
         */
        if ( preg_match("/ <strong>(.*?)\<\/strong><\/p> <\/div>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<div class=\"timg\"> <span>(.*?)<\/span> <\/div>/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"script\">[\s]*(.*?)<\/div> <script/", $string, $m) ) {
			$content = $m[1];
