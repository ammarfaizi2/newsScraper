 ...
 * @version 0.0.1
 */
final class Beritasumut implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Authors
         */
        if ( preg_match("/\(([a-z|A-Z|\s]*?)\)[\s]*<\/div>/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/[\S]+\, ([0-9|A-Z|a-z|\s]*?) <\/div>/", $l["out"], $m) ) {
    ...
    private function formatDate(string $date)
	{
		...
		if( count($expDate) > 1 ) {
			...
			if( isset($this->substrDates[$sub]) ) {
				return $expDate[2].'-'.$this->substrDates[$sub].'-'.$expDate[0].' 00:00:00';
			}
		}
		return false;
	}
	...
	{
		if ( preg_match("/<div class=\"pdlr15_300\">([\s|\S]*?)<div class=\"clear pd10 pt10\"><\/div>/", $string, $m) ) {
            $rawContent = $m[1];
