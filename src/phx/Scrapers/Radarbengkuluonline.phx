...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Radarbengkuluonline as RadarbengkuluonlineProcessor;
 ...
 * @version 0.0.1
 */
final class Radarbengkuluonline extends NewsScraper
{

	const targetList = [
        "Bengkulu" => "https://www.radarbengkuluonline.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RadarbengkuluonlineProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.radarbengkuluonline\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
