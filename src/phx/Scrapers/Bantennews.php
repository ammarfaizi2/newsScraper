...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Bantennews as BantennewsProcessor;
 ...
 * @version 0.0.1
 */
final class Bantennews extends NewsScraper
{

	const targetList = [
        "Banten" => "https://www.bantennews.co.id/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BantennewsProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				...
				preg_match_all(
					"/href=\"(https\:\/\/www\.bantennews\.co\.id\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
