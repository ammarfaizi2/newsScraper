...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarmakassar as KabarmakassarProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarmakassar extends NewsScraper
{

	const targetList = [
        "Sulawesi Selatan" => "https://www.kabarmakassar.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarmakassarProcessor($this->host . $url, $this);
