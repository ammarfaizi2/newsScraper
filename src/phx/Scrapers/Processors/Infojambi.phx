 ...
 * @version 0.0.1
 */
final class Infojambi implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://infojambi.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<strong>Laporan (.*?)<\/strong>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode($o[1], ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }

        if ( preg_match("/<strong>Editor \: (.*?)<\/strong>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode($o[1], ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        ...
        // Get date and time
        if ( preg_match_all("/datetime=\"(.*?)\+/", $l["out"], $m) ) {
			$time = $m[1][0];
	...
	{
		if( preg_match("/\"articleBody\">([\s|\S]+)<div class=\"betteradscontainer/", $string, $m) ) {
            $content = $m[1];
