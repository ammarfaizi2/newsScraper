...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Riausky as RiauskyProcessor;
 ...
 * @version 0.0.1
 */
final class Riausky extends NewsScraper
{

	const targetList = [
        "Riau" => "https://riausky.com/news/indeks"
	];

    private $host = "https://riausky.com/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RiauskyProcessor($url, $this);
