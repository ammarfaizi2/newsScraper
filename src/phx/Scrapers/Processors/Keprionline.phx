 ...
 * @version 0.0.1
 */
final class Keprionline implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://keprionline.co.id/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"entry-title\">[\s|\t|\n]+(.*?)[\s|\t|\n]+<\/h1>/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/published\" datetime=\"(.*?)\+/", $l["out"], $m) ) {
			$time = $m[1];
	...
	{
		// if( preg_match("/<div class=\"entry-content\">([\s|\S]+)<div class=\"post-views/", $string, $m) ) {
        //     $content = $m[1];
