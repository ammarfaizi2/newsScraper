...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Tanjungpinangpos as TanjungpinangposProcessor;
 ...
 * @version 0.0.1
 */
final class Tanjungpinangpos extends NewsScraper
{

	const targetList = [
        "Kepulauan Riau" => "http://www.tanjungpinangpos.co.id/"
	];

    private $host = "http://www.tanjungpinangpos.co.id/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new TanjungpinangposProcessor($url, $this);
