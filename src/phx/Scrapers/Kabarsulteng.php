...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarsulteng as KabarsultengProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarsulteng extends NewsScraper
{

	const targetList = [
        "Sulawesi Tengah" => "https://kabarsulteng.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarsultengProcessor($url, $this);
