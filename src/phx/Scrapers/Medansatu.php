...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Medansatu as MedansatuProcessor;
 ...
 * @version 0.0.1
 */
final class Medansatu extends NewsScraper
{

	const targetList = [
        "Sumatera Utara" => "https://medansatu.com/berita/category/sumatera-utara/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new MedansatuProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/<header><h2 class=\"post-title\"><a href=\"(.*?)\">/",
					$currentPage,
					$n
				)
			) {
