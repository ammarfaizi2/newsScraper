...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Matatelinga as MatatelingaProcessor;
 ...
 * @version 0.0.1
 */
final class Matatelinga extends NewsScraper
{

	const targetList = [
        "Sumatera Utara" => "http://matatelinga.com/Berita-Sumut"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new MatatelingaProcessor($url, $this);
