...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Jogja as JogjaProcessor;
 ...
 * @version 0.0.1
 */
final class Jogja extends NewsScraper
{

	const targetList = [
        "Daerah Istimewa Yogyakarta" => "http://www.jogja.co/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new JogjaProcessor( $url, $this);
