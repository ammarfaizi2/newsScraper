...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Radarsultengonline as RadarsultengonlineProcessor;
 ...
 * @version 0.0.1
 */
final class Radarsultengonline extends NewsScraper
{

	const targetList = [
        "Sulawesi Tengah" => "http://radarsultengonline.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RadarsultengonlineProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/radarsultengonline\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
