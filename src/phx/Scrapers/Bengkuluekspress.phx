...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Bengkuluekspress as BengkuluekspressProcessor;
 ...
 * @version 0.0.1
 */
final class Bengkuluekspress extends NewsScraper
{

	const targetList = [
        "Bengkulu" => "https://bengkuluekspress.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BengkuluekspressProcessor($url, $this);
