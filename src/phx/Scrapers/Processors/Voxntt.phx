 ...
 * @version 0.0.1
 */
final class Voxntt implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<b>Oleh: ([^\<]+)<\/b>/", $l["out"], $o) ) {
	...
	{
		if( preg_match("/<\/h1>([\s|\S]*?)<footer>/", $string, $m) ) {
			$content = $m[1];
