 ...
 * @version 0.0.1
 */
final class Sulutpost implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://sulutpost.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/rel=\"author\" class=\"url fn n\" itemprop=\"url\"><span itemprop=\"name\">(.*?)<\/span>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        } else {
            $author = trim(html_entity_decode(strip_tags("Sulutpost"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        //     $author = trim(html_entity_decode(strip_tags("Sulutpost"), ENT_QUOTES, "UTF-8"));
        ...
        if ( preg_match("/<time class=\"entry-published updated\" datetime=\"([^\+]+)\+/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"entry-the-content\">([\s|\S]*?)<div class=\"screen-reader-text\"/", $string, $m) ) {
			$content = $m[1];
			...
			if( preg_match_all("/<p[^\>]*>([\s\S]*?)<\/p>/", $content, $o) ) {
				foreach ($o[1] as $key => $value) {
                    $next = $key + 1;
					$value = str_replace("komentar", "", $value);
