 ...
 * @version 0.0.1
 */
final class Riauterkini implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/color=\"brown\"><b>(.*?)<\/b><\/font><font/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/color=\"yellow\">[\S]+\, (.*?)<\/font>/", $l["out"], $m) ) {
            $time = $m[1];
            $exp = explode(' ', $time);
            $mon = $exp[1];
            ...
            if( isset($this->months[$sub_3]) ) {
                $real_mon = sprintf("%02d", $this->months[$sub_3]);
                $time = $exp[2] . '/' . $real_mon . '/' . sprintf("%02d", $exp[0]) . ' ' . $exp[3] . ':00';
            }
	...
	{
		if( preg_match("/<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" align=\"center\"[ width=\"99\%\"]*>([\s|\S]*?)<\/table>[\s|\n|\t]*<\!-- AddThis/", $string, $m) ) {
            $content = preg_replace("/<font[^>]+>(.*?)<\/font>/", "", $m[1]);
