...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Aktual as AktualProcessor;
 ...
 * @version 0.0.1
 */
final class Aktual extends NewsScraper
{

	const targetList = [
        "DKI Jakarta" => "http://www.aktual.com/jakarta-raya/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new AktualProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/www\.aktual\.com\/[^\"]+)\" rel=\"bookmark\"/",
					$currentPage,
					$n
				)
			) {
