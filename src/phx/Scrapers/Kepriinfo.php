...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kepriinfo as KepriinfoProcessor;
 ...
 * @version 0.0.1
 */
final class Kepriinfo extends NewsScraper
{

	const targetList = [
        "Kepulauan Riau" => "https://kepri.info/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KepriinfoProcessor($url, $this);
