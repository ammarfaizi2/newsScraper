...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritajateng as BeritajatengProcessor;
 ...
 * @version 0.0.1
 */
final class Beritajateng extends NewsScraper
{

	const targetList = [
        "Jawa Tengah" => "http://beritajateng.net/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritajatengProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/beritajateng\.net\/[^\"]+)\" rel=\"bookmark\"/",
					$currentPage,
					$n
				)
			) {
