 ...
 * @version 0.0.1
 */
final class Papuanews implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://papuanews.id/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"name post-title entry-title\" itemprop=\"itemReviewed\"[^\>]+>(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/rel=\"author\">([^\<]+)<\/a>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Papuanews"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
	...
	{
		if( preg_match("/<div class=\"entry\">([\s\S]*?)<div id=\"wpdevar_comment_1\"/", $string, $m) ) {
			$content = $m[1];
