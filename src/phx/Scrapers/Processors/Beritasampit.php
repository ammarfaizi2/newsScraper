 ...
 * @version 0.0.1
 */
final class Beritasampit implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.rmolkalbar.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<p>EDITOR \: (.*?)<\/p>/", $l["out"], $o) ) {
        ...
        if ( preg_match("/og:image\" content=\"([^\"]+)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<figcaptionn class=\"wp-caption-text\">(.*?)<\/figcaption>/", $l["out"], $n) ) {
	...
	{
		if( preg_match("/<div class=\"td-post-content\">([\s|\S]*?)<div id=\'mye_param\'/", $string, $m) ) {
			$content = $m[1];
