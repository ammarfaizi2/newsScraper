...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Sultrakini as SultrakiniProcessor;
 ...
 * @version 0.0.1
 */
final class Sultrakini extends NewsScraper
{

	const targetList = [
        "Sulawesi Tenggara" => "https://sultrakini.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new SultrakiniProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/sultrakini\.com\/berita\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
