 ...
 * @version 0.0.1
 */
final class Beritajateng implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://beritajateng.net/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/class=\"td-author-by\">By<\/div> <a href=\"[^\"]+\">(.*>)<\/a>/", $l["out"], $o) ) {
        ...
        if( preg_match_all("/<li><a href=\"http\:\/\/beritajateng\.net\/tag\/[^\"]+\">(.*?)<\/a><\/li>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
