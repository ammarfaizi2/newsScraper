...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarpadang as KabarpadangProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarpadang extends NewsScraper
{

	const targetList = [
        "Sumatera Barat" => "http://www.kabarpadang.com/indeks/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarpadangProcessor($url, $this);
