 ...
 * @version 0.0.1
 */
final class Rmol implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 id=\"intro\">(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        // if ( preg_match("/<span>Penulis: (.*?)<\/span>/", $l["out"], $o) ) {
        //     $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // } else {
        //     $author = trim(html_entity_decode(strip_tags("Rmol"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
        ...
        if ( preg_match("/og\:image\" content=\"([^\"]+)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<span class=\"news-picture-caption\">(.*?)<\/span>/", $l["out"], $n) ) {
		...
		if ( preg_match("/<i class=\"fa fa-clock-o\"><\/i>[^\,]+\, (.*?) \, (.*?) WIB /", $l["out"], $m) ) {
        ...
        // if ( preg_match("/og\:url\" content=\"http\:\/\/www\.rmol\.co\/read\/([0-9]+\/[0-9]+\/[0-9]+)\/[0-9]+\//", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"newsContentOne\">([\s|\S]*?)<div class=\"share-article-body shareWrap\">/", $string, $m) ) {
			$content = $m[1];
