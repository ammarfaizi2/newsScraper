 ...
 * @version 0.0.1
 */
final class Kabarbabel implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.kabarbabel.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/Penulis<\/div> <a href=\"http\:\/\/www\.kabarbabel\.com\/author\/(.*?)\/\"/", $l["out"], $o) ) {
         ...
         * Get Images
         */
        if ( preg_match_all("/og\:image\" content=\"(.*?)\" \/>/", $l["out"], $m) ) {

            $img = trim(html_entity_decode($m[1][0], ENT_QUOTES, "UTF-8"));
			icelog("Got image: " . $img);
            $des = "";

            if( preg_match("/<figcaption class=\"wp-caption-text\">(.*?)<\/figcaption>/", $l["out"], $m) ) {
        ...
        if ( preg_match_all("/<figure[^>]+>(.+)<\/figure>/", $l["out"], $m) ) {

            foreach ($m[1] as $k => $v) {

                if( preg_match("/src=\"(.*?)\"/", $v, $p) ) {
                    $img = trim(html_entity_decode($p[1], ENT_QUOTES, "UTF-8"));
                    icelog("Got image: " . $img);
                    $des = "";
        ...
        if( preg_match_all("/a href=\"http\:\/\/www\.kabarbabel\.com\/tag\/[.]+\/\">(.*?)<\/a>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
         ...
         * Get post type
         */
        if( preg_match("/property=\"og\:type\" content=\"(.*?)\" \//", $l["out"], $m) ) {
	...
	{
		// if( preg_match("/<div class=\"entry-content entry-content-single\" itemprop=\"text\">(.*?)<\/div><footer class=\"entry-footer\">/", $string, $m) ) {
        //     $content = $m[1];

            if( preg_match_all("/<p[^>]*>(.*?)<\/p>/", $string, $n) ) {
                foreach ($n[1] as $k => $v) {
