...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Republika as RepublikaProcessor;
 ...
 * @version 0.0.1
 */
final class Republika extends NewsScraper
{

	const targetList = [
        "Jawa Barat" => "https://www.republika.co.id/kanal/koran/kabar-jabar"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RepublikaProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/republika\.co\.id\/berita\/koran\/kabar-jabar\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
