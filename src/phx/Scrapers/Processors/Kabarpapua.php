 ...
 * @version 0.0.1
 */
final class Kabarpapua implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://kabarpapua.co/";
	...
	public function run(): bool
	{
         ...
         * 
         */
        // if ( preg_match("/<a rel=\"author\" href=\"[^\"]+\">([^\<]+)<\/a>/", $l["out"], $o) ) {
        //     $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // } else {
        //     $author = trim(html_entity_decode(strip_tags("Kabarpapua"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
		...
		// if ( preg_match("/<span class=\"itemDateCreated\">[^\,]+\, (.*?)[\s\t]+<\/span>/", $l["out"], $m) ) {
        ...
        //             $time = $exp[2] . '/' . $real_mon . '/' . sprintf("%02d", $exp[0]) . ' ' . $exp[3];
        //         }
        //     }
        ...
        if( preg_match_all("/<li><a href=\"\/itemlist\/tag\/[^\"]+\">([^\<]+)<\/a><\/li>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
	...
	{
		if( preg_match("/<div class=\"td-post-content\">([\s\S]*?)<div class=\"at-below-post/", $string, $m) ) {
			$content = $m[1];
