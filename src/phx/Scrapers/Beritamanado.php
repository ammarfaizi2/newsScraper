...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritamanado as BeritamanadoProcessor;
 ...
 * @version 0.0.1
 */
final class Beritamanado extends NewsScraper
{

	const targetList = [
        "Sulawesi Utara" => "http://beritamanado.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritamanadoProcessor($url, $this);
