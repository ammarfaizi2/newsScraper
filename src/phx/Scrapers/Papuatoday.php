...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Papuatoday as PapuatodayProcessor;
 ...
 * @version 0.0.1
 */
final class Papuatoday extends NewsScraper
{

	const targetList = [
        "Papua" => "https://www.papuatoday.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new PapuatodayProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.papuatoday\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
