 ...
 * @version 0.0.1
 */
final class Deliknews implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.deliknews.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"post-title entry-title\">(.*?)<\/h1>/", $l["out"], $m) ) {
         ...
         * 
         */
        if ( preg_match("/class=\"author-name\" title=\"([^\"]+)\"/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        } else {
            $author = trim(html_entity_decode(strip_tags("Deliknews"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
	...
	{
		if( preg_match("/<div class=\"entry-content entry clearfix\">([\s\S]*?)<div class=\"stream-item stream-item-below-post-content\">/", $string, $m) ) {
			$content = $m[1];
