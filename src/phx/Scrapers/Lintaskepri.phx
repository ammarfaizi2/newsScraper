...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Lintaskepri as LintaskepriProcessor;
 ...
 * @version 0.0.1
 */
final class Lintaskepri extends NewsScraper
{

	const targetList = [
        "Kepulauan Riau" => "https://lintaskepri.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new LintaskepriProcessor($url, $this);
