 ...
 * @version 0.0.1
 */
final class Tribunmaluku implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.tribun-maluku.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/Laporan [^\,]+\, ([^\<]+)[\s\t\n]+</", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Tribun Maluku"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        
        //     $author = trim(html_entity_decode(strip_tags("Tribunmaluku"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
	...
	{
		if( preg_match("/<div class=\"content-inner([\s\S]*?)class=\"jeg_share_bottom_container\"/", $string, $m) ) {
			$content = $m[1];
