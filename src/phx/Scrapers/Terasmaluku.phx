...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Terasmaluku as TerasmalukuProcessor;
 ...
 * @version 0.0.1
 */
final class Terasmaluku extends NewsScraper
{

	const targetList = [
        "Maluku" => "https://terasmaluku.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new TerasmalukuProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/terasmaluku\.com\/[^\"]+)\" rel=\"bookmark\"/",
					$currentPage,
					$n
				)
			) {
