...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Infokepri as InfokepriProcessor;
 ...
 * @version 0.0.1
 */
final class Infokepri extends NewsScraper
{

	const targetList = [
        "Kepulauan Riau" => "http://www.infokepri.com/"
	];

    private $host = "http://www.infokepri.com/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new InfokepriProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\'(http\:\/\/www\.infokepri\.com\/[0-9]+\/[0-9]+\/[a-zA-Z\-]*\.html+)\'/",
					$currentPage,
					$n
				)
			) {
