...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarjakarta as KabarjakartaProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarjakarta extends NewsScraper
{

	const targetList = [
        "DKI Jakarta" => "https://www.kabarjakarta.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarjakartaProcessor($this->host . $url, $this);
