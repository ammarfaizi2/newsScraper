 ...
 * @version 0.0.1
 */
final class Lensapapua implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.lensapapua.com/";
	...
	public function run(): bool
	{
         ...
         * 
         */
        if ( preg_match("/name=\"author\" content=\"([^\"]+)\"/", $l["out"], $o) ) {
        ...
        //     $author = trim(html_entity_decode(strip_tags("Lensapapua"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
        ...
        if ( preg_match("/datePublished\" content=\"([^\+]+)\+/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"entry\">([\s\S]*?)<center style=\"font-weight:/", $string, $m) ) {
			$content = $m[1];
