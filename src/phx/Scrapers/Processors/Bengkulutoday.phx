 ...
 * @version 0.0.1
 */
final class Bengkulutoday implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.bengkulutoday.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"post-title\">[\s\t]*(.*?)[\s\t]*<\/h1>/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/dateCreated\" content=\"(.*?)\+/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div property=\"schema\:text\">([\s|\S]*?)<div id=\"node-single-comment\">/", $string, $m) ) {
            $content = $m[1];
