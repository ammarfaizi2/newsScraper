 ...
 * @version 0.0.1
 */
final class Beritasumbar implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
		...
		if ( preg_match("/<title>(.*?)<\/title>/", $l["out"], $m) ) {
			$this->title = trim(html_entity_decode( str_replace(' | Berita Sumbar', '', $m[1]), ENT_QUOTES, "UTF-8"));
         ...
         * Get Authors
         */
        if ( preg_match("/Pewarta \:<\/div> <a href=\"[\S]+\">(.*?)<\/a>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/title=\"[^\"]+\" class=\"\" \/>[^<span>]+<span>(.*?)<\/span>/", $l["out"], $m) ) {
         ...
         * Get post type
         */
        if( preg_match("/og\:type\" content=\"(.*?)\" \/><meta property=\"og\:description\"/", $l["out"], $m) ) {
	...
	private function doContent(string $string)
	{
		if( preg_match_all("/<p>(.*?)<\/p>/", $string, $m) ) {
            foreach ($m[1] as $k => $v) {
                if( ! preg_match("/<strong>Baca juga\:<\/strong>/", $v, $n) ) {
