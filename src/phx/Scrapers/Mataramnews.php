...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Mataramnews as MataramnewsProcessor;
 ...
 * @version 0.0.1
 */
final class Mataramnews extends NewsScraper
{

	const targetList = [
        "Nusa Tenggara Barat" => "https://mataramnews.co.id/nusa-tenggara-barat?start=1695"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new MataramnewsProcessor( $this->host . $url, $this);
