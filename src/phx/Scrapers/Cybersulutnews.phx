...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Cybersulutnews as CybersulutnewsProcessor;
 ...
 * @version 0.0.1
 */
final class Cybersulutnews extends NewsScraper
{

	const targetList = [
        "Sulawesi Utara" => "http://cybersulutnews.co.id/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new CybersulutnewsProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/cybersulutnews\.co\.id\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
