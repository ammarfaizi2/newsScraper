...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Sijorikepri as SijorikepriProcessor;
 ...
 * @version 0.0.1
 */
final class Sijorikepri extends NewsScraper
{

	const targetList = [
        "Kepulauan Riau" => "http://sijorikepri.com/category/sijori-terkini/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new SijorikepriProcessor($url, $this);
