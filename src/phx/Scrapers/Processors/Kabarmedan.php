 ...
 * @version 0.0.1
 */
final class Kabarmedan implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
        ...
        */
        ...
        if ( preg_match("/\"og\:image\" content=\"(.*?)\?fit/", $l["out"], $m) ) {
            ...
            if( preg_match("/td-modal-image\" src=\"[\s]+\" alt=\"[\s*]\" title=\"(.*?)\"\/>/", $l["out"], $m) ) {
        ...
        // Get Tags 
        if ( preg_match("/<div class=\"ol-tags-single\">[\s|\t|\n]*(.*?)[\s|\t|\n]*<\/div>/", $l["out"], $m) ) {
            $contentTags = $m[1];
