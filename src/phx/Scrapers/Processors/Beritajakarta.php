 ...
 * @version 0.0.1
 */
final class Beritajakarta implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.beritajakarta.id/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1  class=\"hidden-xs hidden-sm\">(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/<small>Reporter \: (.*?) \|/", $l["out"], $o) ) {
	...
	{
		if( preg_match("/thumb-read\">([\s|\S]*?)<div class=\"row padding-top\">/", $string, $m) ) {
            $content = $m[1];
