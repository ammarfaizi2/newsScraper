...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Mimoza as MimozaProcessor;
 ...
 * @version 0.0.1
 */
final class Mimoza extends NewsScraper
{

	const targetList = [
        "Gorontalo" => "http://mimoza.tv/category/kota-gorontalo/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new MimozaProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/class=\"title\" href=\"(http\:\/\/mimoza\.tv\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
