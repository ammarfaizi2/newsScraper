 ...
 * @version 0.0.1
 */
final class Suaramanado implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.suaramanado.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<span>Penulis: (.*?)<\/span>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Suaramanado"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        ...
        // Get date and time
        if ( preg_match("/<span id=\"news-date\" class=\"news-date\">[^\,]+\, (.*?) \&ndash\; (.*?) WITA<\/span>/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div id=\"news-content\">([\s|\S]*?)<div id=\"news-footer\">/", $string, $m) ) {
			$content = $m[1];
