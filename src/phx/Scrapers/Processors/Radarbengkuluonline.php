 ...
 * @version 0.0.1
 */
final class Radarbengkuluonline implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.radarbengkuluonline.com";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<meta name=\"author\" content=\"([^\"]+)\">/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $exp    = explode('/', $author);
            foreach ($exp as $key => $value) {
                if( $value != "" ) {
                    $this->authors[] = $value;
                    icelog("Got author: " . $value);
                }
            }
        }
         ...
         * Get Images
         */
        if ( preg_match("/og\:image\" content=\"(.*?)\" \/><meta name=\"author\"/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"td-post-content td-pb-padding-side\">([\s|\S]*?)<div id='jp-relatedposts' class='jp-relatedposts' >/", $string, $m) ) {
            $content = $m[1];
