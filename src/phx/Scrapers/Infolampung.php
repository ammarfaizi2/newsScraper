...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Infolampung as InfolampungProcessor;
 ...
 * @version 0.0.1
 */
final class Infolampung extends NewsScraper
{

	const targetList = [
        "Lampung" => "http://infolampung.co/"
	];

    private $host = "http://infolampung.co/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new InfolampungProcessor($this->host . $url, $this);
