 ...
 * @version 0.0.1
 */
final class Lintaskepri implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://lintaskepri.com";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/Editor \: (.*?)<\/strong>/", $l["out"], $o) ) {
        ...
        // Get date and time
        if ( preg_match("/class=\"entry-date updated\" datetime=\"(.*?)\+/", $l["out"], $m) ) {
			$time = $m[1];
        ...
        // Get Tags 
        if ( preg_match("/<ul class=\"tag-list\">(.*?)<\/ul>/", $l["out"], $m) ) {
            $contentTags = $m[1];
	...
	private function doContent(string $string)
	{
        ...
        //     $content = $m[1];
            ...
            if( preg_match_all("/<p>(.*?)<\/p>/", $string, $n) ) {
                foreach ($n[1] as $k => $v) {
                    if( preg_match("/<img(.*?)>/", $v, $o) ) {
                        if( preg_match("/src=\"(.*?)\"/", $o[1], $p) ) {
                            $des = "";
                            $img = $p[1];
                            $this->images[] = array(
                                'url' => $img,
