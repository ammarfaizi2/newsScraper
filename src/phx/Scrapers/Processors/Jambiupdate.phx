 ...
 * @version 0.0.1
 */
final class Jambiupdate implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.jambiupdate.co/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<strong>[\s]*\((.*?)\)<\/strong>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode($o[1], ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        ...
        // Get date and time
        if ( preg_match("/<b>[\S]+\, (.*?) WIB<\/b>/", $l["out"], $m) ) {
            $time = $m[1];
	...
	{
		if( preg_match("/<div class=\'content-news\'>([\s|\S]+)<\/div>[\s|\t|\n]*<div class=\'properties\'>/", $string, $m) ) {
            $content = $m[1];
