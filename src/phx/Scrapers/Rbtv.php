...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Rbtv as RbtvProcessor;
 ...
 * @version 0.0.1
 */
final class Rbtv extends NewsScraper
{

	const targetList = [
        "Bengkulu" => "http://rbtv.co.id/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RbtvProcessor($url, $this);
