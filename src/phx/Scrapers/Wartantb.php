...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Wartantb as WartantbProcessor;
 ...
 * @version 0.0.1
 */
final class Wartantb extends NewsScraper
{

	const targetList = [
        "Nusa Tenggara Barat" => "http://www.wartantb.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new WartantbProcessor( $url, $this);
