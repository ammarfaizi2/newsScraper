 ...
 * @version 0.0.1
 */
final class Vifapost implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://vifapost.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 itemprop=\"headline\" class=\"entry-title\">(.*?)<\/h1>/", $l["out"], $m) ) {
         ...
         * 
         */
        if ( preg_match("/Laporan [^\,]+\, ([^\<]+)[\s\t\n]+</", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        } else {
            $author = trim(html_entity_decode(strip_tags("Admin"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        
        //     $author = trim(html_entity_decode(strip_tags("Vifapost"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
        ...
        if ( preg_match("/og\:image\"[\s]+content=\"([^\"]+)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<figcaption[^\>]*>(.*?)<\/figcaption>/", $l["out"], $n) ) {
        ...
        if ( preg_match_all("/datePublished\" content=\"([^\+]+)\+/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"entry-content clearfix\">([\s\S]*?)<div class=\"post-views/", $string, $m) ) {
			$content = $m[1];
