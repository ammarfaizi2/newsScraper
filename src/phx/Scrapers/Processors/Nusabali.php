 ...
 * @version 0.0.1
 */
final class Nusabali implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.nusabali.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/Penulis \: <span itemprop=\"author\">([^\<]+)<\/span>/", $l["out"], $o) ) {
        ...
        if ( preg_match("/og:image\" content=\"([^\"]+)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<span class=\"pull-right\">(.*?)<\/span>/", $l["out"], $n) ) {
	...
	{
		if( preg_match("/<div class=\"entry-content\"([\s|\S]*?)<div class=\"page-links-wrapper\">/", $string, $m) ) {
			$content = $m[1];
