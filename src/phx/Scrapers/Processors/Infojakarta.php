 ...
 * @version 0.0.1
 */
final class Infojakarta implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"post-title\">(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/[0-9]+ by <a href=\"[\"]+\">(.*?)<\/a>/", $l["out"], $o) ) {
	...
	private function doContent(string $string)
	{
		if( preg_match("/<article[^\>]+>([\s|\S]*?)<\/article>/", $string, $m) ) {
            ...
            if ( preg_match_all("/<img([^\>]+)+>/", $string, $m) ) {

                foreach ($m[1] as $key => $value) {
                    $img = "";
                    $des = "";

                    if( preg_match("/src=\"([^\"]+)\"/", $value, $n) ) {
                        $img = trim(html_entity_decode($n[1], ENT_QUOTES, "UTF-8"));
                        icelog("Got image: " . $img);
                    }
        
                    if( preg_match("/alt=\"([^\"]+)\"/", $value, $n) ) {
