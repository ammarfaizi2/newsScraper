 ...
 * @version 0.0.1
 */
final class Mimoza implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h2 itemprop=\"name\" class=\"entry-title post-title\">(.*?)<\/h2>/", $l["out"], $m) ) {
        ...
        //     $author = trim(html_entity_decode(strip_tags("Mimoza"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
        ...
        if( preg_match_all("/rel=\"category tag\">(.*?)<\/a>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
	...
	{
		if( preg_match("/<p class=\"post-meta\">([\s\S]*?)<div class=\"pvc_clear\"><\/div>/", $string, $m) ) {
			$content = $m[1];
