...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Batamtoday as BatamtodayProcessor;
 ...
 * @version 0.0.1
 */
final class Batamtoday extends NewsScraper
{

	const targetList = [
        "Kepulauan Riau" => "http://batamtoday.com/home/indeks"
	];

    private $host = "http://batamtoday.com/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BatamtodayProcessor($url, $this);
