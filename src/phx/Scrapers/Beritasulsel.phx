...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritasulsel as BeritasulselProcessor;
 ...
 * @version 0.0.1
 */
final class Beritasulsel extends NewsScraper
{

	const targetList = [
        "Sulawesi Selatan" => "http://berita-sulsel.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritasulselProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/berita-sulsel\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
