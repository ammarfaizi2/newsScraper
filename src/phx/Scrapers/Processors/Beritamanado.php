 ...
 * @version 0.0.1
 */
final class Beritamanado implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://beritamanado.com/";
	...
	public function run(): bool
	{
        ...
        // if ( preg_match("/Reporter \: <a href=\'[^\']+\'>([^\<]+)<\/a>/", $l["out"], $o) ) {
        //     $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // } else {
        //     $author = trim(html_entity_decode(strip_tags("Beritamanado"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
        ...
        if ( preg_match("/og\:image\" content=\"([^\"]+)\"\/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<div class=\'img-float-left-caption\'>(.*?)<\/div>/", $l["out"], $n) ) {
        ...
        // Get date and time
        if ( preg_match("/<div class=\"calendar-date\">(.*?)<\/div><div class=\"calendar-time\"> <font>(.*?)<\/font> <font>(.*?)<\/font>/", $l["out"], $m) ) {

            $month_date     = $m[1];
            $detail_time    = $m[2] . ':00';
            $year           = $m[3];

            $exp    = explode(" ", $month_date);
			
            $detail_time = "00:00:00";
            
            if( count($exp) > 1 AND count($exp) <= 4 ) {
			...
			$time 	= $this->toWIB($time);
            
            $this->datetime = $time;
			icelog("Got time: " . $time);
        }

        // if ( preg_match("/og\:url\" content=\"http\:\/\/www\.rmolkalbar\.com\/read\/([0-9]+\/[0-9]+\/[0-9]+)\/[0-9]+\//", $l["out"], $m) ) {

        //     $time   = $m[1];

        //     $detail = "00:00:00";

        //     if( preg_match("/[0-9]+ \, (.*?) WIB  \| <b>/", $l["out"], $m) ) {
        //         $detail = $m[1];
        //     }
        ...
        // if ( preg_match_all("/datePublished" content="2018-10-15T14:21:35+/", $l["out"], $m) ) {
        ...
        /* if( preg_match_all("/<li><a href=\"[^\"]+\">(.*?)<\/a><\/li>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
	...
	{
		if( preg_match("/class=\"shortcode-content\">([\s|\S]*?)<div class=\"textwidget\">/", $string, $m) ) {
			$content = $m[1];

			// if ( preg_match_all("/<figure[^\>]*>(.*?)<\/figure>/", $content, $m) ) {
			...
			// 		if( preg_match("/<figcaption[^\>]*>(.*?)<\/figcaption>/", $value, $de) ) {
			...
			if( preg_match_all("/<p[^\>]*>([\s\S]*?)<\/p>/", $content, $o) ) {
				foreach ($o[1] as $key => $value) {
                    $next = $key + 1;
					$value 	= str_replace("Baca Juga :", "", $value);
					$this->content .= trim(html_entity_decode(strip_tags($value) . "\n", ENT_QUOTES, "UTF-8"));
                    icelog("Got content");
                    // if( ! isset($o[1][$next]) ) {
                        if ( preg_match("/\(<strong>([^\<]+)<\/strong>\)/", $value, $p) ) {
