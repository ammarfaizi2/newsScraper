 ...
 * @version 0.0.1
 */
final class Beritakalteng implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://beritakalteng.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"name post-title entry-title\">(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/<i class=\"fa fa-user\"><\/i><a href=\"https\:\/\/beritakalteng\.com\/author\/([^\/]+)\/\"/", $l["out"], $o) ) {
         ...
         * Get Images
         */
        if ( preg_match("/og:image\" content=\"([^\"]+)\" \/>[\s\t\n]+<link/", $l["out"], $m) ) {
            ...
            if( preg_match("/data-caption=\"([^\"]+)\"/", $l["out"], $n) ) {
        ...
        // Get date and time
        // if ( preg_match("/<small style=\"color\:\#00AEEF\;\">[^\,]+\, (.*?)<\/small>/", $l["out"], $m) ) {
        ...
        //             $time = $exp[2] . '/' . $real_mon . '/' . sprintf("%02d", $exp[0]) . ' ' . $detail_time . ':00';
        //         }
        //     }
        ...
        if ( preg_match("/article\:published_time\" content=\"(.*?)\+/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"entry\">([\s|\S]*?)<div class=\"sharedaddy/", $string, $m) ) {
			$content = $m[1];
