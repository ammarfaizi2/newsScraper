...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarsumut as KabarsumutProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarsumut extends NewsScraper
{

	const targetList = [
        "Sumatera Utara" => "http://www.kabarsumut.com/category/sumut-2/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarsumutProcessor($url, $this);
