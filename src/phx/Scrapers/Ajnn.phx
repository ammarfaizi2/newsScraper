...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Ajnn as AjnnProcessor;
 ...
 * @version 0.0.1
 */
final class Ajnn extends NewsScraper
{

	const targetList = [
        "Nanggroe Aceh Darussalam" => "http://www.ajnn.net/news/index.html"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new AjnnProcessor($url, $this);
