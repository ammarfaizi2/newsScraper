...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Gosumbar as GosumbarProcessor;
 ...
 * @version 0.0.1
 */
final class Gosumbar extends NewsScraper
{

	const targetList = [
        "Sumatera Barat" => "https://www.gosumbar.com/berita/sumatera-barat/"
	];

    private $host = "https://www.gosumbar.com";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new GosumbarProcessor($this->host . $url, $this);
