 ...
 * @version 0.0.1
 */
final class Jakartakita implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://jakartakita.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<span class=\"post-title\" itemprop=\"headline\">(.*?)<\/span>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/class=\"post-author-name\">By <b>(.*?)<\/b>/", $l["out"], $o) ) {
        ...
        if ( preg_match("/og:image\" content=\"([\"]+)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<div style=\"color\: \#666\;\"><small>(.*?)<\/small><\/div>/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"entry-content clearfix single-post-content\">([\s|\S]*?)<div class=\"sharedaddy sd-sharing-enabled\">/", $string, $m) ) {
            $content = $m[1];
