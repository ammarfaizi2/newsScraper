...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Fajaronline as FajaronlineProcessor;
 ...
 * @version 0.0.1
 */
final class Fajaronline extends NewsScraper
{

	const targetList = [
        "Sulawesi Barat" => "http://fajaronline.co.id/sulawesi-barat",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new FajaronlineProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/fajaronline\.co\.id\/read\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
