...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Lensapapua as LensapapuaProcessor;
 ...
 * @version 0.0.1
 */
final class Lensapapua extends NewsScraper
{

	const targetList = [
        "Papua" => "https://www.lensapapua.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new LensapapuaProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.lensapapua\.com\/[^\"]+\/)\" rel=\"bookmark\"/",
					$currentPage,
					$n
				)
			) {
