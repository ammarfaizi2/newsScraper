 ...
 * @version 0.0.1
 */
final class Medansatu implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/\"og\:image\" content=\"(.*?)\"\/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<div class=\"caption\">(.*?)<\/div>/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/published updated\" datetime=\"(.*?)\+/", $l["out"], $m) ) {
	...
	{
		if ( preg_match("/\"><\/div><p>([\S|\s]*?)<\/p><div style=\"clear\:both\">/", $string, $m) ) {
            $rawContent = $m[1];
            $rawContent = preg_replace("/<div ([\s|\S]*?)<\/div>/", "", $rawContent);

            if( preg_match("/\(medansatu\.com\/(.*?)\)/", $rawContent, $n) ) {
