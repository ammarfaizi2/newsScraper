 ...
 * @version 0.0.1
 */
final class Sulbarkita implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://sulbarkita.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<strong>([^\<]+)<br \/>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Sulbarkita"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        //     $author = trim(html_entity_decode(strip_tags("Sulbarkita"), ENT_QUOTES, "UTF-8"));
        ...
        if ( preg_match("/og\:image\"[\s]+content=\"([^\"]+)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<figcaption[^\>]*>([\s\S]*?)<\/figcaption>/", $l["out"], $n) ) {
	...
	{
		if( preg_match("/<div class=\"text\">([\s|\S]*?)<div class=\"shareBox\">/", $string, $m) ) {
			$content = $m[1];
