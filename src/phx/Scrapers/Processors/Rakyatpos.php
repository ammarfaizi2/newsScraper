 ...
 * @version 0.0.1
 */
final class Rakyatpos implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.rakyatpos.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/name=\"og:title\" content=\"(.*?)\" \//", $l["out"], $m) ) {
        ...
        if ( preg_match("/author\:username\" content=\"(.*?)\"/", $l["out"], $o) ) {
        ...
        if( preg_match_all("/class=\"tags\">\#(.*?)<\/a>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
