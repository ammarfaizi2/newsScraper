...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Satusulteng as SatusultengProcessor;
 ...
 * @version 0.0.1
 */
final class Satusulteng extends NewsScraper
{

	const targetList = [
        "Sulawesi Tengah" => "http://www.satusulteng.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new SatusultengProcessor($url, $this);
