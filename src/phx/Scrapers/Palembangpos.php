...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Palembangpos as PalembangposProcessor;
 ...
 * @version 0.0.1
 */
final class Palembangpos extends NewsScraper
{

	const targetList = [
        "Sumatera Selatan" => "http://palembang-pos.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new PalembangposProcessor($url, $this);
