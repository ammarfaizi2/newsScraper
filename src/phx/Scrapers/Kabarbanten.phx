...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarbanten as KabarbantenProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarbanten extends NewsScraper
{

	const targetList = [
        "Banten" => "https://www.kabar-banten.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarbantenProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.kabar\-banten\.com\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
