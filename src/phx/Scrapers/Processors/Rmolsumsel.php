 ...
 * @version 0.0.1
 */
final class Rmolsumsel implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.rmolsumsel.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/[\S]+\, (.*?) WIB/", $l["out"], $m) ) {
            ...
            $exp    = explode(" ", $time);
            if( count($exp) > 1 ) {
                $lower = strtolower($exp[1]);
                $sub_3 = substr($lower, 0, 3);
                if( isset($this->months[$sub_3]) ) {
                    $mon = $this->months[$sub_3];
                    $real_mon = sprintf("%02d", $mon);
	...
	{
		if( preg_match("/<div class=\"post newsContent\">([\s|\S]*?)<div class=\"banner-336x250\" align=\"center\">/", $string, $m) ) {
            $content = $m[1];
            $content = preg_replace("/<div class=\"newsAds\">([\s|\S]*?)<span class=\"hidden-xs\">[\s]+<\/span>[\s]+<\/div>/", "", $content);
