 ...
 * @version 0.0.1
 */
final class Portalsatu implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/class=\"h1\">(.*?)<\/span>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)\"\/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/class=\"text-credit-desk\">(.*?)<\/figcaption>/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/<li class=\"small\"> (.*?) WIB<\/li>/", $l["out"], $m) ) {
			$datetime = $this->formatDate($m[1]);
			if( $datetime != false ) {
	...
	{
		if ( preg_match("/<div class=\"text-content\">([\s|\S]*?)<\/div>/", $string, $m) ) {
            $rawContent = $m[1];
