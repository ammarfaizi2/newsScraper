...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Infojambi as InfojambiProcessor;
 ...
 * @version 0.0.1
 */
final class Infojambi extends NewsScraper
{

	const targetList = [
        "Jambi" => "https://infojambi.com/indexed/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new InfojambiProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/<h2 class=\"title\"><a class=\"post-url\" itemprop=\"url\" rel=\"bookmark\" href=\"(.*?)\"/",
					$currentPage,
					$n
				)
			) {
