 ...
 * @version 0.0.1
 */
final class Beritamalutcom implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://beritamalut.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/Oleh: ([^\<]+)[\s\t\n]+<\/p>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Beritamalutcom"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
	...
	{
		if( preg_match("/<div class=\"entry-content entry-content-single\" itemprop=\"text\">([\s\S]*?)<footer/", $string, $m) ) {
			$content = $m[1];
