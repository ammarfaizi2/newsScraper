 ...
 * @version 0.0.1
 */
final class Radargorontalo implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://radargorontalo.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"entry-title\">([\s\S]*?)<\/h1>/", $l["out"], $m) ) {
        ...
        // if ( preg_match("/Reporter : (.*?) \|/", $l["out"], $o) ) {
        ...
        //     $author = trim(html_entity_decode(strip_tags("Radargorontalo"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
        ...
        if( preg_match_all("/article\:tag\" content=\"([^\"]+)\" \/>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
	...
	{
		if( preg_match("/<div class=\"entry-content clearfix\">([\s|\S]*?)<div class=\"tptn_counter/", $string, $m) ) {
			$content = $m[1];
