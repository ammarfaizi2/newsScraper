...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Mediasulbar as MediasulbarProcessor;
 ...
 * @version 0.0.1
 */
final class Mediasulbar extends NewsScraper
{

	const targetList = [
        "Sulawesi Barat" => "http://mediasulbar.co.id/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new MediasulbarProcessor($url, $this);
