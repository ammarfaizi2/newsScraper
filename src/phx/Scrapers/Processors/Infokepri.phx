 ...
 * @version 0.0.1
 */
final class Infokepri implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://kepri.co.id/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/content=\'(.*?)\' property=\'og\:title\'/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/published timeago\' title=\'(.*?)\+/", $l["out"], $m) ) {
			$time = $m[1];
	...
	private function doContent(string $string)
	{
        ...
        if( preg_match_all("/<div style=\"text-align\: justify\;\">[\s|\t|\n]*(.*?)[\s|\t|\n]*<\/div>/", $string, $n) ) {
