...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Okezone as OkezoneProcessor;
 ...
 * @version 0.0.1
 */
final class Okezone extends NewsScraper
{

	const targetList = [
        "Sulawesi Selatan" => "https://www.okezone.com/tag/sulawesi-selatan",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new OkezoneProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/news\.okezone\.com\/read\/[0-9]+\/[0-9]+\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
