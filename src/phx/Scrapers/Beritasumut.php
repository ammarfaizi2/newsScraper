...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritasumut as BeritasumutProcessor;
 ...
 * @version 0.0.1
 */
final class Beritasumut extends NewsScraper
{

	const targetList = [
        "Sumatera Utara" => "http://Beritasumut.com/Berita-Sumut"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritasumutProcessor($url, $this);
