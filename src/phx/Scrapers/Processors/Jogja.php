 ...
 * @version 0.0.1
 */
final class Jogja implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.jogja.co/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h2 class=\"item-title\">(.*?)<\/h2>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/rel=\"author\"><\/a> in (.*?)<\/span>/", $l["out"], $o) ) {
	...
	{
		if( preg_match("/<div class=\"item-content\">([\s|\S]*?)<h3 class=\"jp-relatedposts-headline\">/", $string, $m) ) {
			$content = $m[1];
