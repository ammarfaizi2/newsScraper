 ...
 * @version 0.0.1
 */
final class Republika implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.republika.co.id/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1>(.*?)<\/h1>[\s\t\n]+<div class=\"date_detail\">/", $l["out"], $m) ) {
        ...
        if ( preg_match("/<strong>ed<\/strong>\: (.*?)<\/p>/", $l["out"], $o) ) {
	...
	{
		if( preg_match("/<div class=\"artikel\" itemprop = \"articleBody\">([\s|\S]*?)<div class=\"wrap_sosmed\">/", $string, $m) ) {
            $content = $m[1];
