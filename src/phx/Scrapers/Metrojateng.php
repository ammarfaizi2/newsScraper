...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Metrojateng as MetrojatengProcessor;
 ...
 * @version 0.0.1
 */
final class Metrojateng extends NewsScraper
{

	const targetList = [
        "Jawa Tengah" => "https://metrojateng.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new MetrojatengProcessor( $url, $this);
