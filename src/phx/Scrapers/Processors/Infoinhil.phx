 ...
 * @version 0.0.1
 */
final class Infoinhil implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
        ...
        // Get date and time
        if ( preg_match("/<time datetime=\"(.*?)\+07\:00\" itemprop=\"datePublished\">/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"article-content-main\">([\s|\S]+)<\!-- Perfect Link with Article/", $string, $m) ) {
            $content = $m[1];
