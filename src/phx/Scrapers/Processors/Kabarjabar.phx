 ...
 * @version 0.0.1
 */
final class Kabarjabar implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://kabarjabar.com/";
	...
	public function run(): bool
	{
        ...
        // if( preg_match_all("/<li><h1><a href=\"[^\"]+\" title=\"republika\">(.*?)<\/a><\/h1><\/li>/", $l["out"], $n) ) {
        //     foreach ($n[1] AS $key => $tag) {
        //         $this->tags[] = ucwords($tag);
        //         icelog("Got tag: " . ucwords($tag));
        //     }
	...
	{
		if( preg_match("/<\/span>[\s]+<\/header>([\s|\S]*?)<footer>/", $string, $m) ) {
			$content = $m[1];
