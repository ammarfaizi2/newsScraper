...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Tempo as TempoProcessor;
 ...
 * @version 0.0.1
 */
final class Tempo extends NewsScraper
{

	const targetList = [
        "Sulawesi Tengah" => "https://www.tempo.co/tag/pemerintah-provinsi-sulawesi-tengah",
		"Papua Barat" => "https://www.tempo.co/tag/papua-barat"
	];

    private $host = "https://www.tempo.co/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new TempoProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
             ...
             */
			if( preg_match(
				"/href=\"(http\:\/\/beritabangka\.com\/[a-zA-Z0-9\-\_]+\/)\"/",
				$this->html,
				$m
			) ) {
			...
			if(
				preg_match_all(
					"/<div class=\"card card-type-1\">[\s\t\n]+<div class=\"wrapper clearfix\">[\s\t\n]+<a class=\"col\" href=\"(https\:\/\/[^\"]*tempo\.co\/read\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
