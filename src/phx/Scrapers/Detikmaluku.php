...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Detikmaluku as DetikmalukuProcessor;
 ...
 * @version 0.0.1
 */
final class Detikmaluku extends NewsScraper
{

	const targetList = [
        "Maluku" => "https://detikmaluku.com/category/berita-daerah/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new DetikmalukuProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/detikmaluku\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
