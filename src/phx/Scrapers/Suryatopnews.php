...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Suryatopnews as SuryatopnewsProcessor;
 ...
 * @version 0.0.1
 */
final class Suryatopnews extends NewsScraper
{

	const targetList = [
        "Sulawesi Barat" => "http://suryatopnews.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new SuryatopnewsProcessor($url, $this);
