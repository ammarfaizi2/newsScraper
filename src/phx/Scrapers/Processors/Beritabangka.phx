 ...
 * @version 0.0.1
 */
final class Beritabangka implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/author\:first_name\" content=\"(.*?)\" \//", $l["out"], $o) ) {
	...
	{
		if( preg_match("/<div class=\"entry-content entry-content-single\" itemprop=\"text\">([\s|\S]+)<\!-- Simple Share/", $string, $m) ) {
		    $content = $m[1];
		
			if ( preg_match("/<p[^>]*>Reporter : (.*?)<\/p>/", $content, $o) ) {
				$author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
				$this->authors[] = $author;
				icelog("Got author: " . $author);
			}
