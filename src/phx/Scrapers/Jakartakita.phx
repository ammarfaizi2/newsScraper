...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Jakartakita as JakartakitaProcessor;
 ...
 * @version 0.0.1
 */
final class Jakartakita extends NewsScraper
{

	const targetList = [
        "DKI Jakarta" => "http://jakartakita.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new JakartakitaProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/jakartakita\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
