...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kupastuntas as KupastuntasProcessor;
 ...
 * @version 0.0.1
 */
final class Kupastuntas extends NewsScraper
{

	const targetList = [
        "Lampung" => "https://www.kupastuntas.co/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KupastuntasProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.kupastuntas\.co\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
