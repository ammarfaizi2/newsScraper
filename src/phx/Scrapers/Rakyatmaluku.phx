...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Rakyatmaluku as RakyatmalukuProcessor;
 ...
 * @version 0.0.1
 */
final class Rakyatmaluku extends NewsScraper
{

	const targetList = [
        "Maluku" => "http://rakyatmaluku.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RakyatmalukuProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/rakyatmaluku\.com\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
