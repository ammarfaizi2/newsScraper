...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kepricoid as KepricoidProcessor;
 ...
 * @version 0.0.1
 */
final class Kepricoid extends NewsScraper
{

	const targetList = [
        "Kepulauan Riau" => "https://kepri.co.id/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KepricoidProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/<a href=\"(https\:\/\/kepri\.co\.id\/[0-9]+\/[0-9]+\/[0-9]+\/.+)\"/",
					$currentPage,
					$n
				)
			) {
