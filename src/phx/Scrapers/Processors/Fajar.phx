 ...
 * @version 0.0.1
 */
final class Fajar implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://radarsulbar.fajar.co.id/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"post-title entry-title left\" itemprop=\"name headline\">(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/rel=\"author\">(.*?)<\/a>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Fajar"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        //     $author = trim(html_entity_decode(strip_tags("Fajar"), ENT_QUOTES, "UTF-8"));
		...
		if ( preg_match("/datePublished\" datetime=\"([^\"]+)\">[^\,]+\, [0-9]+ \@(.*?)<\/time>/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"post-feat-text\">([\s|\S]*?)\_footer \_footer/", $string, $m) ) {
			$content = $m[1];
