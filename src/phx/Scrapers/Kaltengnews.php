...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kaltengnews as KaltengnewsProcessor;
 ...
 * @version 0.0.1
 */
final class Kaltengnews extends NewsScraper
{

	const targetList = [
        "Kalimantan Tengah" => "https://kaltengnews.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KaltengnewsProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/kaltengnews\.com\/[0-9]+\-[0-9]+\/)\"/",
					$currentPage,
					$n
				)
			) {
