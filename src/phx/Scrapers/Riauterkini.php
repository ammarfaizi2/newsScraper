...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Riauterkini as RiauterkiniProcessor;
 ...
 * @version 0.0.1
 */
final class Riauterkini extends NewsScraper
{

	const targetList = [
        "Riau" => "http://riauterkini.com/inxberita.php"
	];

    private $host = "http://riauterkini.com/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RiauterkiniProcessor($this->host . $url, $this);
