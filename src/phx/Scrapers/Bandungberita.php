...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Bandungberita as BandungberitaProcessor;
 ...
 * @version 0.0.1
 */
final class Bandungberita extends NewsScraper
{

	const targetList = [
        "Jawa Barat" => "https://bandungberita.com/category/info-jabar/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BandungberitaProcessor($url, $this);
