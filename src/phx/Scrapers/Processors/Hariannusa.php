 ...
 * @version 0.0.1
 */
final class Hariannusa implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://hariannusa.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/published_time\" content=\"(.*?)\+00:00\" \//", $l["out"], $m) ) {
        ...
        // if( preg_match_all("/<li><a href=\"http\:\/\/www\.wartantb\.com\/tag\/[^\"]+\">(.*?)<\/a><\/li>/", $l["out"], $n) ) {
        //     foreach ($n[1] AS $key => $tag) {
        //         $this->tags[] = ucwords($tag);
        //         icelog("Got tag: " . ucwords($tag));
        //     }
	...
	{
		if( preg_match("/class=\"dable-content-wrapper\">([\s|\S]*?)<div id=\"wpdevar_comment_2\"/", $string, $m) ) {
			$content = $m[1];
