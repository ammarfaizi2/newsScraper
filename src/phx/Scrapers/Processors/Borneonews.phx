 ...
 * @version 0.0.1
 */
final class Borneonews implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<i class=\"fa fa-user\"><\/i> Oleh (.*?)[\s\t\n]+<\/li>/", $l["out"], $o) ) {
        ...
        if ( preg_match_all("/\&q\;dateprop\&q\;\:\&q\;([^\&]+)\&q\;/", $l["out"], $m) ) {
	...
	{
		// if( preg_match("/<div class=\"content\">([\s|\S]*?)<div class=\"selanjutnya-container\">/", $string, $m) ) {
			// $content = $m[1];
			...
			if( preg_match_all("/<p innerhtml=\"([^\"]+)\">/", $content, $o) ) {
