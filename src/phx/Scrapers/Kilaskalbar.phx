...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kilaskalbar as KilaskalbarProcessor;
 ...
 * @version 0.0.1
 */
final class Kilaskalbar extends NewsScraper
{

	const targetList = [
        "Kalimantan Barat" => "https://kilaskalbar.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KilaskalbarProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/kilaskalbar\.com\/[^\/]+\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
