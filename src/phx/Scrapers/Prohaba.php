...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Prohaba as ProhabaProcessor;
 ...
 * @version 0.0.1
 */
final class Prohaba extends NewsScraper
{

	const targetList = [
        "Nanggroe Aceh Darussalam" => "http://www.prohaba.co/index-news/all"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new ProhabaProcessor($url, $this);
