...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Radarlampung as RadarlampungProcessor;
 ...
 * @version 0.0.1
 */
final class Radarlampung extends NewsScraper
{

	const targetList = [
        "Lampung" => "https://radarlampung.co.id/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RadarlampungProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/radarlampung\.co.id\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
