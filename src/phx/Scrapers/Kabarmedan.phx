...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarmedan as KabarmedanProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarmedan extends NewsScraper
{

	const targetList = [
        "Sumatera Utara" => "https://kabarmedan.com/category/sumatera-utara/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarmedanProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/td-module-title\"><a href=\"(.*?)\" rel=\"bookmark\"/",
					$currentPage,
					$n
				)
			) {
