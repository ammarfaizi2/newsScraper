 ...
 * @version 0.0.1
 */
final class Jambiekspres implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://metrojambi.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)\"\/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/class=\"tagline-top[^>]+>([\s|\S]+)<\/div>[\s\t\n]+<div class=\"contentberita left\"> /", $l["out"], $m) ) {
        ...
        if ( preg_match("/[\S]+\, (.*?)[\s]+- <i>/", $l["out"], $m) ) {
            ...
            if( count($exp) > 1 ) {
                $mon = trim($exp[1]);
                $lower = strtolower($mon);
                $sub_3 = substr($lower, 0, 3);
                if( isset($this->months[$sub_3]) ) {
                    $real_mon = $this->months[$sub_3];
                    $real_mon = sprintf("%02d", $real_mon);
                    $time = $exp[2] . '/' . $real_mon . '/' . $exp[0] . ' ' . $exp[4];
                }
            }
        ...
        if( preg_match_all("/<div class=\"taglink\">[\s|\t|\n]*(.*?)[\s|\t|\n]*<\/div>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
	...
	{
		if( preg_match("/<div class=\"right contentx editor-content\">([\s|\S]+)<div class=\"clear\"><\/div>/", $string, $m) ) {
            $content = $m[1];

            if( preg_match_all("/<p[^>]*>(.*?)<\/p>/", $string, $n) ) {
                foreach ($n[1] as $key => $value) {
                    if ( preg_match("/\(([a-zA-Z\/]*?)\)/", $value, $o) ) {
                        $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
                        $this->authors[] = $author;
                        icelog("Got author: " . $author);
                    }
