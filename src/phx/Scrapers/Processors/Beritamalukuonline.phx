 ...
 * @version 0.0.1
 */
final class Beritamalukuonline implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.beritamalukuonline.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class='post-title single-title emfont' itemprop='name'>(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/Oleh: ([^\<]+)[\s\t\n]+<\/p>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Beritamalukuonline"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        ...
        if ( preg_match("/content='([^\']+)' property='og:image'/", $l["out"], $m) ) {
            ...
            if( preg_match("/<p class=\"wp-caption-text\">(.*?)<\/p>/", $l["out"], $n) ) {
		...
		// if ( preg_match("/<i class=\"icon-calendar\"><\/i> ([A-Za-z]+) ([0-9]+)\, ([0-9]+)<\/span>/", $l["out"], $m) ) {
        ...
        //             $time = $year . '/' . $real_mon . '/' . sprintf("%02d", $date) . ' 00:00:00';
        //         }
        //     // }
         ...
         * Get post type
         */
        if( preg_match("/name=\"og\:type\"[\s\t]+content=\"([^\"]+)\" \//", $l["out"], $m) ) {
	...
	{
		if( preg_match("/itemprop='articleBody'>([\s\S]*?)<div class='tags sprites'>/", $string, $m) ) {
			$content = $m[1];
