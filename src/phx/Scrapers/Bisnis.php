...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Bisnis as BisnisProcessor;
 ...
 * @version 0.0.1
 */
final class Bisnis extends NewsScraper
{

	const targetList = [
        "Jawa Timur" => "http://surabaya.bisnis.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BisnisProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/surabaya\.bisnis\.com\/read\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
