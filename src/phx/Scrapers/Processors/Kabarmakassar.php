 ...
 * @version 0.0.1
 */
final class Kabarmakassar implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.kabarmakassar.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<div class=\"news-detil\">[\s\t\n]+<h1>(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/Oleh (.*?)<\/span>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        
        //     $author = trim(html_entity_decode(strip_tags("Kabarmakassar"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
	...
	{
		if( preg_match("/<div class=\"read-news\">([\s|\S]*?)<div class=\"footer-social\">/", $string, $m) ) {
			$content = $m[1];
