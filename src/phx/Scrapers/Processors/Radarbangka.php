 ...
 * @version 0.0.1
 */
final class Radarbangka implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://radarbangka.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/rel=\"author\">(.*?)<\/a>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Radar Bangka"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
	...
	{
		if( preg_match("/description\"\: \"([\s|\S]+)\"[\s|\t|\n]+\}/", $string, $m) ) {
            $content = $m[1];
