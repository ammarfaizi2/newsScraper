 ...
 * @version 0.0.1
 */
final class Balipost implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.balipost.com/";
	...
	public function run(): bool
	{
        ...
        // Get date and time
        // if ( preg_match("/og\:url\" content=\"https:\/\/www.beritabali.com\/read\/(.{10})/", $l["out"], $m) ) {
        ...
        // if( preg_match_all("/class=\"btn btn-sm btn-default\">(.*?)<\/span>/", $l["out"], $n) ) {
        //     foreach ($n[1] AS $key => $tag) {
        //         $this->tags[] = ucwords($tag);
        //         icelog("Got tag: " . ucwords($tag));
        //     }
	...
	{
		if( preg_match("/<\/figure><\/div>([\s|\S]*?)<footer>/", $string, $m) ) {
			$content = $m[1];
