...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritabangka as BeritabangkaProcessor;
 ...
 * @version 0.0.1
 */
final class Beritabangka extends NewsScraper
{

	const targetList = [
        "Bangka Belitung" => "http://beritabangka.com/"
	];

    private $host = "http://beritabangka.com/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritabangkaProcessor($url, $this);
