 ...
 * @version 0.0.1
 */
final class Bantennews implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.bantennews.co.id/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/Oleh<\/div> <a href=\"[^\"]+\">(.*?)<\/a>/", $l["out"], $o) ) {
         ...
         * Get Images
         */
        if ( preg_match("/\/>[\s\t\n]+<meta property=\"og:image\" content=\"([^\"]+)\" \/>/", $l["out"], $m) ) {
        ...
        // Get date and time
        // if ( preg_match("/<i class=\"fa fa-clock-o\"><\/i>(.*?)<\/span>/", $l["out"], $m) ) {
        ...
        //             $time = $exp[2] . '/' . $real_mon . '/' . sprintf("%02d", $exp[0]) . ' 00:00:00';
        //         }
        //     }
        ...
        if ( preg_match("/published_time\" content=\"(.*?)\+/", $l["out"], $m) ) {
            ...
            $this->datetime = $time;
			icelog("Got time: " . $time);
        } else if ( preg_match("/published_time\" content=\"([0-9\T\:\-\/]+)Z/", $l["out"], $m) ) {
        ...
        if( preg_match_all("/<li><a href=\"https\:\/\/www\.bantennews\.co\.id\/tag\/[\"]+\">(.*?)<\/a><\/li>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
	...
	{
		if( preg_match("/<div class=\"td-post-content td-pb-padding-side\">([\s|\S]*?)<footer>/", $string, $m) ) {
			$content = $m[1];
