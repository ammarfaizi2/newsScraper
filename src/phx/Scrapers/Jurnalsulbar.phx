...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Jurnalsulbar as JurnalsulbarProcessor;
 ...
 * @version 0.0.1
 */
final class Jurnalsulbar extends NewsScraper
{

	const targetList = [
        "Sulawesi Barat" => "https://jurnalsulbar.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new JurnalsulbarProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/jurnalsulbar\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
