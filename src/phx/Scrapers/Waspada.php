...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Waspada as WaspadaProcessor;
 ...
 * @version 0.0.1
 */
final class Waspada extends NewsScraper
{

	const targetList = [
        "Sumatera Utara" => "http://waspada.co.id/sumut/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new WaspadaProcessor($url, $this);
