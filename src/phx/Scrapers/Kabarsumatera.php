...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarsumatera as KabarsumateraProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarsumatera extends NewsScraper
{

	const targetList = [
        "Sumatera Selatan" => "http://kabarsumatera.com/category/nusantara/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarsumateraProcessor($url, $this);
