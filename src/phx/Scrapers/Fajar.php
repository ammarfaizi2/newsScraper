...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Fajar as FajarProcessor;
 ...
 * @version 0.0.1
 */
final class Fajar extends NewsScraper
{

	const targetList = [
        "Sulawesi Barat" => "http://radarsulbar.fajar.co.id/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new FajarProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/[a-z\.]*fajar\.co\.id\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
