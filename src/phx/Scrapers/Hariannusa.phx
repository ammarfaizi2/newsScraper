...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Hariannusa as HariannusaProcessor;
 ...
 * @version 0.0.1
 */
final class Hariannusa extends NewsScraper
{

	const targetList = [
        "Nusa Tenggara Barat" => "https://hariannusa.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new HariannusaProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/hariannusa\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
