...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarsumbar as KabarsumbarProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarsumbar extends NewsScraper
{

	const targetList = [
        "Sumatera Barat" => "https://www.kabarsumbar.com"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarsumbarProcessor($url, $this);
