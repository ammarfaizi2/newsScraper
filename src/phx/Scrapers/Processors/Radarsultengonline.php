 ...
 * @version 0.0.1
 */
final class Radarsultengonline implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://radarsultengonline.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/rel=\"author\">([^\<]+)</", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        } else {
            $author = trim(html_entity_decode(strip_tags("Radarsultengonline"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        //     $author = trim(html_entity_decode(strip_tags("Radarsultengonline"), ENT_QUOTES, "UTF-8"));
		...
		// if ( preg_match("/<p>[a-zA-Z\']+\, (.*?) \| ([0-9\:]+)<\/p>/", $l["out"], $m) ) {
