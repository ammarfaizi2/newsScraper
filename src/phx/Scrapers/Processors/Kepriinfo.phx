 ...
 * @version 0.0.1
 */
final class Kepriinfo implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<span><i class=\'icon-user\'><\/i>[\s|\t|\n]+(.*?)<\/span>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode($o[1], ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        } else {
            $author = trim(html_entity_decode("Kepri.info", ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }

        // if ( preg_match("/<strong>\((.*?)\)<\/strong>/", $l["out"], $o) ) {
        //     $author = trim(html_entity_decode($o[1], ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
         ...
         * Get Images
         */
        // if ( preg_match("/src=\"(.*?)\" class=\"attachment-colormag-featured-image/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"entry-content entry-content-single\" itemprop=\"text\">([\s|\S]+)<\/div><\!-- \.entry\-content -->/", $string, $m) ) {
            $content = $m[1];
			...
			if ( preg_match("/<img src=\"(.*?)\"/", $content, $m) ) {
				...
				if( preg_match("/<figcaption class=\"wp-caption-text\">(.*?)<\/figcaption>/", $content, $m) ) {
					$des = trim(html_entity_decode($m[1], ENT_QUOTES, "UTF-8"));
					icelog("Got description: " . $des);
				}
