 ...
 * @version 0.0.1
 */
final class Gosumut implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Authors
         */
        if ( preg_match("/<span>Penulis\:<\/span> (.*?)<\/div>/", $l["out"], $m) ) {
            $author = trim(html_entity_decode($m[1], ENT_QUOTES, "UTF-8"));
			$this->authors[] = $author;
			icelog("Got author: " . $author);
        }
		if ( preg_match("/<strong>Editor<\/strong><\/td><td width=\"10\"><strong>\:<\/strong><\/td><td>(.*?)<\/td>/", $l["out"], $m) ) {
            $author = trim(html_entity_decode($m[1], ENT_QUOTES, "UTF-8"));
			$this->authors[] = $author;
			icelog("Got author: " . $author);
        }
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)\">/", $l["out"], $m) ) {
            ...
            if( preg_match("/<span class=\"caption\">(.*?)<\/span>/", $l["out"], $m) ) {
	...
	{
		if ( preg_match("/<div class=\"news-content\">(.*?)<\/div><\!\-\- END/", $string, $m) ) {
            $rawContent = $m[1];
            $rawContent = preg_replace("/<div class=\"ads-content\">(.*?)<\/div>/", "", $rawContent);
