...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarpapua as KabarpapuaProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarpapua extends NewsScraper
{

	const targetList = [
        "Papua Barat" => "https://kabarpapua.co/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarpapuaProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/kabarpapua\.co\/[^\"]+\/)\" rel=\"bookmark\"/",
					$currentPage,
					$n
				)
			) {
