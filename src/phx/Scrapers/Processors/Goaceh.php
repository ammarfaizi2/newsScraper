 ...
 * @version 0.0.1
 */
final class Goaceh implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Authors
         */
		/* if ( preg_match("/<span>Penulis\:<\/span> (.*?)<\/div><div class/", $l["out"], $m) ) {
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)\"><meta/", $l["out"], $m) ) {
            ...
            if( preg_match("/class=\"caption\">(.*?)<\/span>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/published_time\" content=\"(.*?)\+/", $l["out"], $m) ) {
            ...
            $datetime = trim(html_entity_decode($datetime, ENT_QUOTES, "UTF-8"));
	...
	{
		if ( preg_match("/<\/div><strong>([\s|\S]+)<div style=\"margin\:20px/", $string, $m) ) {
            $rawContent = $m[1];
