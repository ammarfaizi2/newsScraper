 ...
 * @version 0.0.1
 */
final class Bandungberita implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://bandungberita.com/";
	...
	public function run(): bool
	{
        ...
        // if ( preg_match("/Oleh: <span ><a[^\>]+>(.*?)<\/a>/", $l["out"], $o) ) {
        ...
        //     if( preg_match("/class=\"field field-name-field-file-image-title-text[^\>]+>(.*?)[\s\t\n]+<div class=\"col-xs-3\"><\/div>/", $l["out"], $n) ) {
        //         $des = preg_replace("/<([^\>]+)>/", "", $n[1]);
        //         $des = str_replace("<", "", $des);
        //         $des = str_replace(">", "", $des);
        //         $des = trim(html_entity_decode(strip_tags($des), ENT_QUOTES, "UTF-8"));
        ...
        // Get date and time
        if ( preg_match("/<span>(.*?) WIB<\/span>/", $l["out"], $m) ) {
            ...
            if( count($exp) > 1 AND count($exp) <= 4 ) {
                ...
                if( isset($this->months[$mon]) ) {
                    $real_mon = $this->months[$mon];
                    $real_mon = sprintf("%02d", $real_mon);

                    $time = $exp[2] . '/' . $real_mon . '/' . $exp[0] . ' ' . $exp[3] . ':00';
                }
            }
        ...
        // if( preg_match_all("/<a href=\"\/tags\/[^\"]+\"[^\>]+>(.*?)<\/a>/", $l["out"], $n) ) {
        //     foreach ($n[1] AS $key => $tag) {
        //         $this->tags[] = ucwords($tag);
        //         icelog("Got tag: " . ucwords($tag));
        //     }
	...
	{
		if( preg_match("/<div class=\"content-left\">([\s|\S]*?)<footer>/", $string, $m) ) {
            $content = $m[1];
            ...
            if( preg_match_all("/<p[^\>]*>([\s\S]*?)<\/p>/", $content, $n) ) {
                foreach ($n[1] as $k => $v) {
                    ...
                    if( $v != "" ) {
                        ...
                        if( preg_match_all("/src=\"([\"]+)\"/", $v, $o) ) {

                            foreach ($o[1] as $i => $s) {
                                
                                $img = trim(html_entity_decode($s, ENT_QUOTES, "UTF-8"));
                                $des = "";
