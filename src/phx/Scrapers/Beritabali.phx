...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritabali as BeritabaliProcessor;
 ...
 * @version 0.0.1
 */
final class Beritabali extends NewsScraper
{

	const targetList = [
        "Bali" => "https://www.beritabali.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritabaliProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.beritabali\.com\/read\/[0-9]+\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+\.html)\"/",
					$currentPage,
					$n
				)
			) {
