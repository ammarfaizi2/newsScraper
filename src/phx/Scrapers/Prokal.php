...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Prokal as ProkalProcessor;
 ...
 * @version 0.0.1
 */
final class Prokal extends NewsScraper
{

	const targetList = [
        "Kalimantan Tengah" => "http://kalteng.prokal.co/",
        "Kalimantan Timur" => "http://kaltim.prokal.co/"
	];

    private $host = "http://prokal.co/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new ProkalProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\'(http\:\/\/[a-z\.]*prokal\.co\/read\/news\/[^\']+\.html)\'/",
					$currentPage,
					$n
				)
			) {
