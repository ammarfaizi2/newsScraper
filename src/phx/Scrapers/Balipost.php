...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Balipost as BalipostProcessor;
 ...
 * @version 0.0.1
 */
final class Balipost extends NewsScraper
{

	const targetList = [
        "Bali" => "http://www.balipost.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BalipostProcessor( 'http:' . $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(\/\/www\.balipost\.com\/news\/[0-9]+\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+\.html)\"/",
					$currentPage,
					$n
				)
			) {
