 ...
 * @version 0.0.1
 */
final class Minangkabaunews implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
		...
		if ( preg_match("/og\:title\" content=\"(.*?)\" \//", $l["out"], $m) ) {
            $title = str_replace(' - Minangkabaunews','',$m[1]);
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)\" \/>/", $l["out"], $m) ) {
            ...
            if( preg_match("/<div class=keterangan\_foto>(.*?)<\/div>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/\"datePublished\"\: \"[\S]+\, (.*?) \+/", $l["out"], $m) ) {
            ...
            if( isset($this->months[$lower_mon]) ) {
                $mon_num = $this->months[$lower_mon];
                $str_mon = sprintf("%02d", $mon_num);
                $time = $exp[2] . '/' . $str_mon . '/'. $exp[0] . ' ' . $exp[3];
                /* $time = str_replace($mon, $str_mon, $time); */
            }
            $this->datetime = $time;
        }
