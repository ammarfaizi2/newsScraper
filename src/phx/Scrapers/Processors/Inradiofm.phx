 ...
 * @version 0.0.1
 */
final class Inradiofm implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.inradiofm.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class='post-title entry-title' itemprop='name headline'>[\s|\t|\n]*(.*?)[\s|\t|\n]*<\/h1>/", $l["out"], $m) ) {
         ...
         * Get Images
         */
        if ( preg_match("/<link href='(.*?)' rel='image_src'\/>/", $l["out"], $m) ) {

            $img = trim(html_entity_decode($m[1], ENT_QUOTES, "UTF-8"));
			icelog("Got image: " . $img);
            $des = "";

            if( preg_match("/<p class=\"wp-caption-text\">(.*?)<\/p>/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/class='published' itemprop='datePublished' title='(.*?)\+/", $l["out"], $m) ) {
            $time   = $m[1];
        ...
        if( preg_match_all("/rel='tag'># (.*?)<\/a>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
	...
	{
		// if( preg_match("/<div class=\"post_content\">([\s|\S]+)<div class=\"tag-cat\">/", $string, $m) ) {
        //     $content = $m[1];

            if( preg_match_all("/<span style=\"font-family: inherit;\">(.*?)<\/span>/", $string, $n) ) {
                foreach ($n[1] as $k => $v) {
                    $this->content .= trim(html_entity_decode(strip_tags($v) . "\n", ENT_QUOTES, "UTF-8"));
                    icelog("Got content");
                }
            } elseif( preg_match("/<article>([\s|\S]+)<\/article>/", $string, $n) ) {
				$this->content .= trim(html_entity_decode(strip_tags($n[1]) . "\n", ENT_QUOTES, "UTF-8"));
                icelog("Got content");
			}
    ...
    {
        if( preg_match_all("/<div class=\"comment-content\">([\s|\S]+)<\/div><\!-- \#comment-\#\# -->/", $string, $m) ) {
            foreach ($m[1] as $k => $v) {
                $author = "";
                $content = "";
                $datetime = "";
                if( preg_match("/comment-author fn\">(.*?)<\/h4>/", $v, $n) ) {
                    $author = trim(html_entity_decode(strip_tags($n[1]), ENT_QUOTES, "UTF-8"));
                }
                if( preg_match("/comment-text\"><p>(.*?)<\/p>/", $v, $n) ) {
                    $content = trim(html_entity_decode(strip_tags($n[1]), ENT_QUOTES, "UTF-8"));
                }
                if( preg_match("/datetime=\"(.*?)\+/", $v, $n) ) {
                    $time   = str_replace("T", " ", $n[1]);
