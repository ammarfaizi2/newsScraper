...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritapapua as BeritapapuaProcessor;
 ...
 * @version 0.0.1
 */
final class Beritapapua extends NewsScraper
{

	const targetList = [
        "Papua" => "http://berita-papua.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritapapuaProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/berita-papua\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
