...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Radarbangka as RadarbangkaProcessor;
 ...
 * @version 0.0.1
 */
final class Radarbangka extends NewsScraper
{

	const targetList = [
        "Bangka Belitung" => "http://www.radarbangka.co.id/berita"
	];

    private $host = "http://www.radarbangka.co.id";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RadarbangkaProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/www\.radarbangka\.co\.id\/berita\/detail\/.+\.html)\"/",
					$currentPage,
					$n
				)
			) {
