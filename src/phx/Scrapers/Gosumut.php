...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Gosumut as GosumutProcessor;
 ...
 * @version 0.0.1
 */
final class Gosumut extends NewsScraper
{

	const targetList = [
        "Sumatera Utara" => "https://www.gosumut.com/berita/sumatera-utara/"
	];
    ...
    private $host = "https://www.gosumut.com";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {".$this->host.$url."} ");
					$this->processor = new GosumutProcessor($this->host.$url, $this);
