...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Manadoline as ManadolineProcessor;
 ...
 * @version 0.0.1
 */
final class Manadoline extends NewsScraper
{

	const targetList = [
        "Sulawesi Utara" => "http://manadoline.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new ManadolineProcessor($url, $this);
