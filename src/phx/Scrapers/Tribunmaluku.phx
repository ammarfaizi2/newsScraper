...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Tribunmaluku as TribunmalukuProcessor;
 ...
 * @version 0.0.1
 */
final class Tribunmaluku extends NewsScraper
{

	const targetList = [
        "Maluku" => "https://www.tribun-maluku.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new TribunmalukuProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.tribun-maluku\.com\/[0-9]+\/[0-9]+\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
