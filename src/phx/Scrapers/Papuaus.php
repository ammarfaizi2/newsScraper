...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Papuaus as PapuausProcessor;
 ...
 * @version 0.0.1
 */
final class Papuaus extends NewsScraper
{

	const targetList = [
        "Papua" => "https://www.papua.us/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new PapuausProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href='(https\:\/\/www\.papua\.us\/[0-9]+\/[0-9]+\/[^\']+\.html)\'/",
					$currentPage,
					$n
				)
			) {
