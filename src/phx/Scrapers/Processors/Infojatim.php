 ...
 * @version 0.0.1
 */
final class Infojatim implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://www.infojatim.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class='post-title entry-title' itemprop='name headline'>([\s\S]+)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/content='([^\']+)' name='author'/", $l["out"], $o) ) {
        ...
        // Get date and time
        // if ( preg_match("/<i class=\"fa fa-calendar\"><\/i> [a-zA-Z\']+\, ([^\<]+) <i/", $l["out"], $m) ) {
        ...
        //             if( preg_match("/<i class=\"fa fa-clock-o\"><\/i> (.*?) WIB/", $l["out"], $n) ) {
        //                 $detail_time = $n[1];
        //             }
        ...
        if ( preg_match("/class=\'published updated\' title=\'([^\+]+)\+/", $l["out"], $m) ) {
        ...
        // if( preg_match_all("/<a href=\"http\:\/\/beritajatim\.com\/tag-berita\/[^\"]+\">(.*?)<\/a>/", $l["out"], $n) ) {
        //     foreach ($n[1] AS $key => $tag) {
        //         $this->tags[] = ucwords($tag);
        //         icelog("Got tag: " . ucwords($tag));
        //     }
	...
	{
		if( preg_match("/itemprop='description articleBody'>([\s|\S]*?)<\/article>/", $string, $m) ) {
			$content = $m[1];
