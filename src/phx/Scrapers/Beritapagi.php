...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritapagi as BeritapagiProcessor;
 ...
 * @version 0.0.1
 */
final class Beritapagi extends NewsScraper
{

	const targetList = [
        "Sumatera Selatan" => "http://beritapagi.co.id/category/sumsel"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritapagiProcessor($url, $this);
