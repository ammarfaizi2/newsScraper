...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Rmolkalbar as RmolkalbarProcessor;
 ...
 * @version 0.0.1
 */
final class Rmolkalbar extends NewsScraper
{

	const targetList = [
        "Kalimantan Barat" => "http://www.rmolkalbar.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RmolkalbarProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/www\.rmolkalbar\.com\/read\/[0-9]+\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
