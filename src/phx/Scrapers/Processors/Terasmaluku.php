 ...
 * @version 0.0.1
 */
final class Terasmaluku implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://terasmaluku.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/Rep: ([^\<]+)[\s\t\n]+<\/p>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Terasmaluku"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
	...
	{
		if( preg_match("/class='heateor_sss_sharing_container([\s\S]*?)<footer>/", $string, $m) ) {
			$content = $m[1];
