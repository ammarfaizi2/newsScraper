...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Terkini as TerkiniProcessor;
 ...
 * @version 0.0.1
 */
final class Terkini extends NewsScraper
{

	const targetList = [
        "Sulawesi Selatan" => "https://makassar.terkini.id/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new TerkiniProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/<h3 class=\"entry-title td-module-title\"><a href=\"(https\:\/\/makassar\.terkini\.id\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
