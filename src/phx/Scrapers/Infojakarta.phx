...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Infojakarta as InfojakartaProcessor;
 ...
 * @version 0.0.1
 */
final class Infojakarta extends NewsScraper
{

	const targetList = [
        "DKI Jakarta" => "http://infojakarta.net/"
	];

    private $host = "http://infojakarta.net/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new InfojakartaProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/infojakarta\.net\/[^\"]+)\" rel=\"bookmark\"/",
					$currentPage,
					$n
				)
			) {
