 ...
 * @version 0.0.1
 */
final class Mediasulbar implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://mediasulbar.co.id/";
	...
	public function run(): bool
	{
        ...
        // if ( preg_match("/<strong>([^\<]+)<br \/>/", $l["out"], $o) ) {
        //     $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // } else {
        //     $author = trim(html_entity_decode(strip_tags("Mediasulbar"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
        //     $author = trim(html_entity_decode(strip_tags("Mediasulbar"), ENT_QUOTES, "UTF-8"));
         ...
         * Get Images
         */
        if ( preg_match("/og\:image\"[\s]+content=\"([^\"]+)\" \/>[\s\t\n]+<meta/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"entry\">([\s|\S]*?)<div class='apss-social-share/", $string, $m) ) {
			$content = $m[1];
