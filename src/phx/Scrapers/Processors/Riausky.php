 ...
 * @version 0.0.1
 */
final class Riausky implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/og\:image\" content=\"(.*?)\">/", $l["out"], $m) ) {
            ...
            if( preg_match("/Ket Foto \:(.*?)<\/span>/", $l["out"], $m) ) {
        ...
        // Get date and time
        if ( preg_match("/datePublished\"\: \"[\S]+\, (.*?) \+/", $l["out"], $m) ) {
            ...
            if( isset($this->months[$sub_3]) ) {
                ...
                $time = $exp[2] . '/' . $real_mon . '/' . $exp[0] . ' ' . $exp[3];
            }
	...
	{
		if( preg_match_all("/<p style=\"text-align\:justify\">(.*?)<\/p>/", $string, $m) ) {
            foreach ($m[1] as $k => $v) {

                $content = preg_replace("/<!--[\s]*(.*?)[\s]*-->/", "", $v);
