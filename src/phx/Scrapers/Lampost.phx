...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Lampost as LampostProcessor;
 ...
 * @version 0.0.1
 */
final class Lampost extends NewsScraper
{

	const targetList = [
        "Lampung" => "http://www.lampost.co/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new LampostProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/www\.lampost\.co\/berita-[^\"]+\.html)\"/",
					$currentPage,
					$n
				)
			) {
