 ...
 * @version 0.0.1
 */
final class Hariansinggalang implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
		...
		if ( preg_match("/og\:title\" content=\"(.*?)\" \//", $l["out"], $m) ) {
            $title = str_replace('- Portal Berita Singgalang','',$m[1]);
			$this->title = trim(html_entity_decode($title, ENT_QUOTES, "UTF-8"));
         ...
         * Get Authors
         */
        if ( preg_match("/<strong>[\.]*\((.*?)\)<\/strong>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/<p class=\"post\-tag\">Tags (.*?)<\/p>/", $l["out"], $m) ) {
            ...
            if( preg_match_all("/<a href=\"[\S]+\" rel=\"tag\">(.*?)<\/a>/", $contentTags, $n) ) {
	...
	{
		if ( preg_match("/<div class=\"td-post-content\">([\s|\S]*?)<\/div>[\s|\n|\t]*<footer>/", $string, $m) ) {
            $rawContent = $m[1];
            if( preg_match_all("/<p>(.*?)<\/p>/", $rawContent, $n) ) {
                foreach($n[1] AS $k => $v) {
                    $this->content .= trim(html_entity_decode(strip_tags($v), ENT_QUOTES, "UTF-8"));
                }
            }
            icelog("Got content ");
        }
