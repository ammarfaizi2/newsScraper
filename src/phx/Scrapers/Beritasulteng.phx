...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritasulteng as BeritasultengProcessor;
 ...
 * @version 0.0.1
 */
final class Beritasulteng extends NewsScraper
{

	const targetList = [
        "Sulawesi Tengah" => "http://www.beritasulteng.id/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritasultengProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/www\.beritasulteng\.id\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+\/)\"/",
					$currentPage,
					$n
				)
			) {
