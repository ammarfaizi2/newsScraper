...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Merdeka as MerdekaProcessor;
 ...
 * @version 0.0.1
 */
final class Merdeka extends NewsScraper
{

	const targetList = [
        "Sulawesi Selatan" => "https://makassar.merdeka.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new MerdekaProcessor($this->host . $url, $this);
