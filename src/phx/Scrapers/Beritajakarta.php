...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritajakarta as BeritajakartaProcessor;
 ...
 * @version 0.0.1
 */
final class Beritajakarta extends NewsScraper
{

	const targetList = [
        "DKI Jakarta" => "http://www.beritajakarta.id/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritajakartaProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(http\:\/\/www\.beritajakarta\.id\/read\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
