 ...
 * @version 0.0.1
 */
final class Bengkuluekspress implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://bengkuluekspress.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"post-title\" itemprop=\"headline\" >[\s\t]*(.*?)[\s\t]*<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/class=\"author-name\">(.*?)<\/span>/", $l["out"], $o) ) {
        ...
        // if( preg_match_all("/class=\"category-tag\" rel=\"bookmark\">(.*?)<\/a>/", $l["out"], $n) ) {
        //     foreach ($n[1] AS $key => $tag) {
        //         $this->tags[] = ucwords($tag);
        //         icelog("Got tag: " . ucwords($tag));
        //     }
         ...
         * Get post type
         */
        if( preg_match("/article\:section\" content=\"(.*?)\" \/>/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"entry-summary\" itemprop=\"text\" >([\s|\S]*?)<div class=\"post-meta-wrapper clearfix\">/", $string, $m) ) {
            $content = $m[1];
