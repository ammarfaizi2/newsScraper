 ...
 * @version 0.0.1
 */
final class Detikmaluku implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://detikmaluku.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<strong>Oleh: ([^\<]+)<\/strong>/", $l["out"], $o) ) {
            ...
            $author = trim(html_entity_decode(strip_tags("Detikmaluku"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
         ...
         * Get Images
         */
        if ( preg_match("/og\:image\"[\s]+content=\"([^\"]+)\" \/><meta/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"td-post-content\">([\s\S]*?)<div class=\"essb_links/", $string, $m) ) {
			$content = $m[1];
