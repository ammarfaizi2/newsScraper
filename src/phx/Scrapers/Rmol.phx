...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Rmol as RmolProcessor;
 ...
 * @version 0.0.1
 */
final class Rmol extends NewsScraper
{

	const targetList = [
        "Sulawesi Utara" => "https://rmol.co/tag.php?tag=sulut",
		"Gorontalo" => "http://rmol.co/tag/gorontalo/1 ",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RmolProcessor($url, $this);
