...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Jambiupdate as JambiupdateProcessor;
 ...
 * @version 0.0.1
 */
final class Jambiupdate extends NewsScraper
{

	const targetList = [
        "Jambi" => "http://www.jambiupdate.co/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new JambiupdateProcessor($this->host . $url, $this);
