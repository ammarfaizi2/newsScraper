...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Hariansinggalang as HariansinggalangProcessor;
 ...
 * @version 0.0.1
 */
final class Hariansinggalang extends NewsScraper
{

	const targetList = [
        "Sumatera Barat" => "https://hariansinggalang.co.id/category/daerah/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new HariansinggalangProcessor($url, $this);
