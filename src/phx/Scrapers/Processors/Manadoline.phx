 ...
 * @version 0.0.1
 */
final class Manadoline implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://manadoline.com/";
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 class=\"entry-title\">(.*?)<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/Penulis<\/div> <a href=\"http\:\/\/manadoline\.com\/author\/[^\"]+\">(.*?)<\/a>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        } else {
            $author = trim(html_entity_decode(strip_tags("Manadoline"), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
         ...
         * Get Images
         */
        if ( preg_match("/og\:image\" content=\"([^\"]+)\"\/><link/", $l["out"], $m) ) {
            ...
            if( preg_match("/<figcaption class=\"wp-caption-text\">(.*?)<\/figcaption>/", $l["out"], $n) ) {
        ...
        if( preg_match_all("/<li><a href=\"http\:\/\/manadoline\.com\/tag\/[^\"]+\">(.*?)<\/a><\/li>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
	...
	{
		if( preg_match("/<div class=\"td-post-content\">([\s|\S]*?)<footer>/", $string, $m) ) {
			$content = $m[1];
