 ...
 * @version 0.0.1
 */
final class Tempo implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	public function run(): bool
	{
         ...
         * Get Title
         */
		if ( preg_match("/<h1 itemprop=\"headline\">[\s\t\n]+(.*?)[\s\t\n]+<\/h1>/", $l["out"], $m) ) {
        ...
        if ( preg_match("/Reporter: <h4 itemprop=\"author\"[^\>]+>(.*?)<\/h4>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }

        if ( preg_match("/Editor: <h4 itemprop=\"editor\"[^\>]+>(.*?)<\/h4>/", $l["out"], $o) ) {
            $author = trim(html_entity_decode(strip_tags($o[1]), ENT_QUOTES, "UTF-8"));
            $this->authors[] = $author;
            icelog("Got author: " . $author);
        }
        
        //     $author = trim(html_entity_decode(strip_tags("Tempo"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
	...
	{
		if( preg_match("/<div id=\"isi\" itemprop=\"articleBody\">([\s|\S]*?)<hr class=\"w-50\">/", $string, $m) ) {
			$content = $m[1];
