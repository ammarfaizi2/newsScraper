...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Reportasebangka as ReportasebangkaProcessor;
 ...
 * @version 0.0.1
 */
final class Reportasebangka extends NewsScraper
{

	const targetList = [
        "Bangka Belitung" => "http://reportasebangka.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new ReportasebangkaProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
             ...
             */
			if( preg_match(
				"/href=\"(http\:\/\/reportasebangka\.com\/berita\/[a-zA-Z0-9\-\_]+)\"/",
				$this->html,
				$m
			) ) {
