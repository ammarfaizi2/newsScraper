...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Visitkepri as VisitkepriProcessor;
 ...
 * @version 0.0.1
 */
final class Visitkepri extends NewsScraper
{

	const targetList = [
        "Kepulauan Riau" => "https://visitkepri.com/"
	];

    private $host = "https://visitkepri.com/";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new VisitkepriProcessor($url, $this);
