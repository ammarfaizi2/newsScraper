 ...
 * @version 0.0.1
 */
final class Beritasulsel implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "http://berita-sulsel.com/";
	...
	public function run(): bool
	{
        ...
        if ( preg_match("/<span>Penulis<\/span><a href=\"[^\"]+\">(.*?)<\/a>/", $l["out"], $o) ) {
        ...
        //     $author = trim(html_entity_decode(strip_tags("Beritasulsel"), ENT_QUOTES, "UTF-8"));
        //     $this->authors[] = $author;
        //     icelog("Got author: " . $author);
        // }
         ...
         * Get Images
         */
        // if ( preg_match("/og\:image\"[\s]+content=\"([^\"]+)\" \/><meta name/", $l["out"], $m) ) {
		...
		// if ( preg_match("/rel=\"canonical\" href=\"http\:\/\/www\.beritasulteng\.id\/([0-9]+\/[0-9]+\/[0-9]+)\//", $l["out"], $m) ) {
        ...
        if ( preg_match("/<time class=\"updated\" datetime=\"([^\+]+)\+/", $l["out"], $m) ) {
	...
	{
		if( preg_match("/<div class=\"entry-content entry-content-single\"([\s|\S]*?)<footer class=\"entry-footer\">/", $string, $m) ) {
			$content = $m[1];

			// if ( preg_match_all("/<figure[^\>]+>([\s\S]*?)<\/figure>/", $content, $m) ) {
