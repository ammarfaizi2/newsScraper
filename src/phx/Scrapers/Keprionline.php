...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Keprionline as KeprionlineProcessor;
 ...
 * @version 0.0.1
 */
final class Keprionline extends NewsScraper
{

	const targetList = [
        "Kepulauan Riau" => "http://keprionline.co.id/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KeprionlineProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/<a href=\"(http:\/\/keprionline.co.id\/[\S]+\/)\" title/",
					$currentPage,
					$n
				)
			) {
