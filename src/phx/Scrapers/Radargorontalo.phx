...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Radargorontalo as RadargorontaloProcessor;
 ...
 * @version 0.0.1
 */
final class Radargorontalo extends NewsScraper
{

	const targetList = [
        "Gorontalo" => "http://radargorontalo.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RadargorontaloProcessor( $url, $this);
