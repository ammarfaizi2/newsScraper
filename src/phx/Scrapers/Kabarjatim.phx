...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kabarjatim as KabarjatimProcessor;
 ...
 * @version 0.0.1
 */
final class Kabarjatim extends NewsScraper
{

	const targetList = [
        "Jawa Timur" => "https://kabarjatim.com/"
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KabarjatimProcessor( $url, $this);
