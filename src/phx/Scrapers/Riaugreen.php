...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Riaugreen as RiaugreenProcessor;
 ...
 * @version 0.0.1
 */
final class Riaugreen extends NewsScraper
{

	const targetList = [
        "Riau" => "http://riaugreen.com/index.html"
	];

    private $host = "http://riaugreen.com";
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new RiaugreenProcessor($url, $this);
