...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Tajuktimur as TajuktimurProcessor;
 ...
 * @version 0.0.1
 */
final class Tajuktimur extends NewsScraper
{

	const targetList = [
        "Maluku" => "https://www.tajuktimur.com/maluku/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new TajuktimurProcessor( $url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/www\.tajuktimur\.com\/maluku\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
