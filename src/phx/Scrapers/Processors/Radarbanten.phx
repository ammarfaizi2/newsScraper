 ...
 * @version 0.0.1
 */
final class Radarbanten implements PhxScraperProcessor
{
	/**
	 * @var string
	 */
	...
	private $host = "https://www.radarbanten.co.id/";
	...
	public function run(): bool
	{
         ...
         * Get Images
         */
        if ( preg_match("/og:image\" content=\"(.*?)\" \/><meta/", $l["out"], $m) ) {
        ...
        if( preg_match_all("/<li><a href=\"https\:\/\/www\.radarbanten\.co\.id\/tag\/[^\"]+\">(.*?)<\/a><\/li>/", $l["out"], $n) ) {
            foreach ($n[1] AS $key => $tag) {
                $this->tags[] = ucwords($tag);
                icelog("Got tag: " . ucwords($tag));
            }
	...
	private function doContent(string $string)
	{
		if( preg_match("/<div class=\"td-post-content\">([\s|\S]*?)<footer>/", $string, $m) ) {
			...
			if( preg_match_all("/<p[^\>]*>(.*?)<\/p>/", $string, $o) ) {
