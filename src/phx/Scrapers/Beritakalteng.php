...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Beritakalteng as BeritakaltengProcessor;
 ...
 * @version 0.0.1
 */
final class Beritakalteng extends NewsScraper
{

	const targetList = [
        "Kalimantan Tengah" => "https://beritakalteng.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new BeritakaltengProcessor($url, $this);
	...
	protected function parseThreadUrlList(string $string): array
	{
		...
		do {
			...
			if(
				preg_match_all(
					"/href=\"(https\:\/\/beritakalteng\.com\/[0-9]+\/[0-9]+\/[0-9]+\/[^\"]+)\"/",
					$currentPage,
					$n
				)
			) {
