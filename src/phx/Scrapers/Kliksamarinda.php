...
use Phx\NewsScraper;
use Phx\Scrapers\Processors\Kliksamarinda as KliksamarindaProcessor;
 ...
 * @version 0.0.1
 */
final class Kliksamarinda extends NewsScraper
{

	const targetList = [
        "Kalimantan Timur" => "http://www.kliksamarinda.com/",
	];
	...
	public function run(): void
	{
		foreach (self::targetList as $key => $url) {
			...
			if (isset($l["error"]) && $l["error"]) {
				...
				foreach($urls as $url) {
					icelog("Scraping {$url} ");
					$this->processor = new KliksamarindaProcessor( $this->host . $url, $this);
